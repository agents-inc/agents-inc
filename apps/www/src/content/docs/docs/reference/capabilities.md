...
| Mint an id from a payload you already hold        | `yes` | `no`   | `cat proposal.json \| npx agents-inc share --stdin`, for anything that produces a configuration but can't apply one.                                                                     |
