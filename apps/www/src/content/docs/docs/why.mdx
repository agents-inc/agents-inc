...
- **Compiled sub-agents** in `.claude/agents/`, one markdown file each. The
  catalogue currently defines {AGENT_COUNT} of them — developers, testers,
  researchers and planners per domain, a single `reviewer` that loads its
  per-domain review skills lazily, and a few meta ones — and you pick which
  to compile.
