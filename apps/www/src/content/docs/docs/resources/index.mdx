...
The official marketplace and the default source the CLI reads from. It holds the skill catalogue itself — {SKILL_COUNT} skills across web, API, AI, mobile, infrastructure, shared tooling and meta practices, each one covering the patterns, conventions, anti-patterns and edge cases for a single technology.
