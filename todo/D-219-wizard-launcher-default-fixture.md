...
- **Hard-error tests.** Tests asserting the "marketplace could not be resolved" path (see `e2e/interactive/edit-plugin-hard-error.e2e.test.ts`) specifically need a source *without* marketplace. Those must remain able to opt out.
