...
| CLI-470 | Uninstall honesty + agent provenance (design settled 2026-08-09, owner: hold): (1) the configless removal plan says compiled agents are KEPT and why; (2) the compiler stamps every agent with a BODY-COMMENT marker on the first line after the frontmatter (`<!-- Generated by agents-inc vX — do not edit; compile rewrites this file -->`) — deliberately NOT a frontmatter field: unknown-key tolerance is undocumented (the 16 supported fields are known; a stricter release would break every install), while the body is free-form by contract, greppable, versioned, and doubles as the do-not-edit notice; (3) uninstall's configless path sweeps by marker — provably ours deleted, unmarked kept, both stated. One-time rewrite ripple on next compile acknowledged. Plus the false os.homedir() comment fix. **Split by the owner 2026-08-10 after the fifth pass reproduced (1):** with an unparseable `config.ts`, `buildRemovalPlan` in `commands/uninstall.tsx` still lists `<agentsDir>/ (CLI-compiled)` under "The following will be removed:", then leaves all 12 files byte-identical — the summary afterwards is honest, the plan the user reads before pressing `y` is not. **Leg (1) — the message only — is queued now**: when the config cannot be read, the agents line moves out of the removal list and says they are KEPT and why (identifying which agents this CLI compiled needs the configuration that is gone). Legs (2) and (3) — the body-comment provenance marker and the marker-driven sweep — stay on hold; they are what would let the configless path actually remove them. | (1) LANDED 2026-08-10 (uncommitted); (2)(3) on hold | bug      | medium     |
