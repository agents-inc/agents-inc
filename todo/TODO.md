...
| ID    | Task                                                                                                                                                                                                                                                                                | Status                   |
| ----- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------ |
| D-241 | `init-dashboard-edit-plugin-install.e2e.test.ts` never exercises genuine project scope (D-226) — fix its HOME setup, update assertion to the already-existing guard's blocked behavior. [Detail](#d-241-init-dashboard-edit-plugin-installe2etestts-never-exercised-genuine-project-scope-d-226) | Ready for Dev            |
...
#### D-241: `init-dashboard-edit-plugin-install.e2e.test.ts` never exercised genuine project scope (D-226)

**Discovered**: a full E2E suite run (2026-07-18, after that day's dual-scope wizard-store.ts
fixes) surfaced `e2e/lifecycle/init-dashboard-edit-plugin-install.e2e.test.ts` failing
reproducibly (confirmed in isolation, not a flake, not caused by that day's changes — see below).

**Repro**: `cc init` installs `web-framework-react` globally (the "Framework" category is
**exclusive** — single-select). Then `dashboard -> Edit`, at what the test labels "project
scope," selects `web-framework-vue-composition-api` (Vue — the other option in the same
exclusive category). **Actual result**: `web-framework-react` disappears from the config
entirely (not even as a tombstone). The test's own assertion expected both react and vue to
survive (coexisting across scopes).

**Root cause — confirmed, not a regression.** The test never sets an explicit `HOME`, so
`TerminalSession` defaults `HOME: cwd`, and `cwd === projectDir` — collapsing
`isEditingFromGlobalScope` to `true` for the "Edit" step (the already-documented D-226 gap; this
test file even has its own `KNOWN GAP` comment acknowledging it). Confirmed two ways: (1) static
trace — today's `wizard-store.ts` diff only touches code paths that are no-ops for this exact
scenario; (2) empirical — swapping in the pre-today `wizard-store.ts` via a plain file copy (no
git operations) reproduced the identical failure, then the working file was restored byte-identical.

**Resolved design question**: in a **genuine** project-scope edit (`isEditingFromGlobalScope=false`),
the existing `exclusive && !isSelected` guard in `toggleTechnology` **already blocks** selecting
vue while react is installed globally (toast: "Global skills cannot be changed from project
scope"). The "you shouldn't be allowed to add a conflicting exclusive-category skill locally"
behavior the user wants **already exists in the store** — no new guard needed. The test only ever
saw react vanish because it was accidentally running as a *global* edit (due to the HOME
collapse), where dropping react outright is the correct global-edit uninstall behavior.

**Fix**: give this test an explicit, separate `HOME` (matching the pattern already used by the
dual-scope tests — `createGlobalOnlyEnv` / explicit `env: { HOME: fakeHome }`) so `isEditingFromGlobalScope`
is correctly `false` for the Edit step, then update the test's assertion to match what the
already-existing guard actually produces: selecting vue while react is globally installed is
BLOCKED (toast shown, react remains, vue never gets added) — not the old "both coexist" expectation.

**Cross-ref**: `todo/D-226-sandbox-home-cwd-collapse.md` (the general gap; this is one concrete,
now-understood instance of it).
