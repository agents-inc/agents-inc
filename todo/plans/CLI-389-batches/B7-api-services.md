...
Scope: worksheet §B7, §2a groups #11/#12/#13, §4; relationship-coverage decisions 2/3; CLI-740 for
...
- **`api-vector-db` — one vector store per service. Radio right.** The strongest coexistence
  ...
  documented as a recommended architecture. Recorded as CLI-740 residue:
...
- **`api-search` — one application-search engine. Radio right, one real dual-role residue.**
  ...
  skill _and_ Meili for search is legitimately blocked. Recorded as CLI-740 residue:
...
together) — exactly the semantics `recommends` had before it was deleted. Handed to CLI-740:
...
fence, which the flip already delivers. CLI-740 line: "payload's full surface (admin panel,
...
CLI-740 nuance worth recording once for the catalog: **React-as-template-renderer is not
...
| chroma (api-vector-db-chroma)                                       | api-vector-db (flip)                   | constrained-via-exclusivity-or-requires                | A                           | []                      | none                                                                                                          | skill body (chromadb v3, auto-embedding); https://docs.trychroma.com                                                                                                                                                           | CLI-740 residue: chroma↔pinecone dev/prod graduation split.                                                                                                                                                                                                                                 |
...
4. **The three flag flips are confirmed as billed** — genuinely one-line, group == whole
   ...
   verified rather than assumed, with two named CLI-740 residues (chroma↔pinecone dev/prod,
...
5. **The setup/usage pairs need no `requires` link — confirmed with product-internal
   ...
   semantics go to CLI-740 as recommends-shaped residue.
...
- Regen round: `generate:types` + `generate:matrix` (+ `generate:schemas` untouched — no
  category ids change).
- CLI-740 residue from this batch: chroma↔pinecone dev/prod split;
