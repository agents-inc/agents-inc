...
Scope: worksheet §B4, §2b group #6, §4; relationship-coverage decision 2; D-306 for anything the
...
states. The cy.mount slice is recorded as D-306 residue, not a fence-breaker — same disposition
...
- D-306 residue: native-WebSocket-to-a-third-party-server beside Socket.IO-to-own-server (the
...
embedding is conceded D-306 residue either way.)
...
pure loosening. A package-health D-306 watch rides the edit: `storybook-framework-qwik` sits
...
a content note, not a narrowing. D-306: a community Astro framework exists
...
## (e) vite and the meta-frameworks — universal, no fence; the anti-fence goes to D-306
...
- **nextjs does not use Vite** (webpack/Turbopack, https://nextjs.org/docs/app/api-reference/turbopack)
  ...
  deletion, so it is recorded as a D-306 line ("vite ⊥ nextjs — and vite ⊥ docusaurus, which is
  ...
  useless, not incoherent) while D-306's dubious-combo lane exists for exactly this.
  **The advice line must name the catalog's own tension (verification finding): 6 of the 15
  ...
  drafted, the D-306 line called the shipped defaults dubious without saying so. Disposition:
  the six rows are **curation debt handed to the stack owner** — either the vite entries come
  out of the six Next rosters, or the pairing is declared intended and the D-306 line gains
  the carve-out.
- **nuxt/sveltekit/astro/qwik/vitepress embed Vite** and expose its config (SvelteKit projects
  ...
  either direction; the embedding nuance rides the same D-306 note.
...
- **visual-regression — universal, class A; harness enumeration DECLINED.** A maestro-shaped
  ...
  — maestro _is_ a tool; this skill is about any tool. Pairing advice → D-306.
...
- **F1 — storybook's rule omits qwik, the sole framework on Storybook's own frameworks page
  ...
  wrong fence; widening is pure loosening. Package-health D-306 watch:
...
- D-306 residue from this batch: cypress-CT-beside-playwright-E2E; native-WS-third-party-feed
