...
2. **Steady-state hybrid: Biome as formatter + a bare ESLint for plugin gaps.** This one is
   ...
   (dual-client-one-provider): radio KEPT, pattern recorded honestly as a D-306 line —
...
| biome (shared-tooling-biome)                                 | shared-lint [X]        | constrained-via-exclusivity-or-requires | A     | []         | none                                                         | skill body (v2.4, `biome check --write`, `biome ci`); https://biomejs.dev/guides/migrate-eslint-prettier/                                                                                                                                                     | Radio vs eslint-prettier held; hybrid recorded for D-306, see disposition.                                                                                                                                                                                           |
...
- **F8 — changesets ↔ `nx release` functional overlap.** The nx skill teaches `nx release`
  ...
  for D-306's "richer semantics" backlog (a `discourages`-shaped fact with no surviving
  vocabulary).
...
4. **Group #16's radio survives Biome's dual-running docs, but not for free** — the honest
   record includes the steady-state Biome+bare-ESLint hybrid as a D-306 line
