...
| `e2e/interactive/sources-step-duplicate-marketplace-column.e2e.test.ts`                                                                                   | **DONE — deleted.** The spec no longer exists; it asserted that two indistinguishable marketplace columns do not render, which a 2-column grid makes impossible. _(Its JSDoc was cited here as evidence for the removal; that evidence went with it.)_ |
...
- This also closes the reported defect where `edit` painted a skill as removed while preserving it — via a third option that was not among the two originally proposed: make the removal _real_. `Changes:` block, `config.ts`, and the compiled agent then all agree, and the pinned `it.fails` spec in `edit-wizard-local.e2e.test.ts` inverts to a positive assertion on the reported reason.
