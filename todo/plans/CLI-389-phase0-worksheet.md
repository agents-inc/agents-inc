...
| 26  | Managed Postgres hosts | neon → `api-baas` (EXCL); vercel-postgres, cockroachdb → `api-database` (EXCL)                      | The known **accepted loss to D-306**. neon sits with supabase/firebase/appwrite/planetscale/turso for good reason; no non-violent restructure recovers the fence. |
