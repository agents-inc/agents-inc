...
describe("edit wizard — pending-removal row when a dual-scope skill collapses to global", () => {
  ...
  it(
    ...
    async () => {
      ...
      // pads its name with spaces (`-  <name> `), which splits the marker from the name, and the
      // processed buffer is repainted in place rather than accumulated, so a form a later frame
      // overwrites is unrecoverable. SourceGrid seeds focus on the first FOCUSABLE row, skipping
      // both inert react rows, so it lands on the untouched vitest row and react's rows keep their
      // markers flush against the name.
