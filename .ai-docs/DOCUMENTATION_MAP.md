...
| E2E Infrastructure      | [DONE] | `reference/testing/e2e-infrastructure.md` | 2026-08-01   | **2026-07-30** | **NEEDS-VALIDATION NARROWED, NOT CLEARED.** `STEP_TEXT` 64 -> **74** and all per-directory spec counts re-derived; POM inventories + all TIMEOUTS/paths still unchecked                      |
...
| Config Writer (Detail)  | [DONE] | `reference/config/config-writer.md`       | 2026-08-01   | **2026-07-30** | PARTIAL 2026-08-01 (`normalizeProjectPath` scope only) — all three sections verified accurate; two spots naming the raw primitive instead of the helper corrected                            |
...
**Source Files:** **350** TypeScript files in `src/cli/` — **184** production (excludes `*.test.*`, `__tests__/`, `__mocks__/`) + **129** test specs (`*.test.*`) + **35** non-spec `__tests__/` support files + **2** `__mocks__/`. (The `__tests__/` directories hold 79 files total: 44 specs + 35 support — **both unchanged**, so the two new specs are co-located, not in `__tests__/`.) **Re-counted with `find` on disk 2026-08-01** (was 347 / 183 / 127 on 2026-07-30). The +1 production file and +2 specs are the 0.147.0 rendering work: `summary-panel.tsx` replaced `info-panel.tsx` (net 0) while `scroll-affordance.tsx`, `wizard-layout.test.tsx` and `summary-panel.test.tsx` are net additions.
**E2E Suite:** **204** TypeScript files in `e2e/` (**167** `*.test.ts(x)` + **37** helpers/fixtures/pages) — **re-counted with `find` on disk 2026-08-01** (was 197 / 160 / 37). Of the 167: **164** `*.e2e.test.ts` + **3** `*.smoke.test.ts`. Per directory, each diffed name-by-name: `commands` 31, `interactive` 54, `lifecycle` 75, `integration` 3, `smoke` 1 e2e + 3 smoke. **The support count is unchanged at 37** — the seven new `interactive` specs are the terminal-resize / confirm-parity / Sources-grid cluster, and everything they needed (`resizeBelowMinimum`, `TERMINAL_SIZE.BELOW_MINIMUM`, `STEP_TEXT.RESIZE_PROMPT`) went into **existing** helper files.
...
**Product Version:** 0.147.1 (`package.json`, verified 2026-08-01)
