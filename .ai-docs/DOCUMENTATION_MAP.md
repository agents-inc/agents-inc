...
**Product Version:** 0.146.0
**Total Areas:** 32 (18 original + 14 new from restructure)
**Documented:** 32 (100%)
**In Progress:** 0
**Needs Validation:** 2 (`testing/e2e-infrastructure.md`, `commands/index.md` — content drift from the 2026-07-30 bug-fix batch; both rows annotated `NEEDS-VALIDATION` below. Their `last_validated` dates are legitimately current, so the staleness dashboard cannot surface this — the flag is the only signal.)
**Last Validated:** 2026-07-30 (**reconciliation pass**, following the same day's five bug fixes and their test coverage. Every count below was **re-counted on disk in this session**; none carried forward. Changes: E2E file counts 196 → 197 and 159 → 160 specs; the agent-findings total **removed from this map entirely** and delegated to its owning doc (`reference/findings-impact-report.md`) per the count-ownership registry — the map had been restating it in three places, all of them stale; `config/config-writer.md` row re-annotated for its new Path-normalization section; findings-impact-report row rebuilt (its annotation was still describing the 2026-07-24 state at 99 findings); two rows flagged `NEEDS-VALIDATION`; new "Known Tooling Gaps" section added. 5-invariant audit re-run with Invariant 4 verified **by name** — see Validation History. Prior — index-consistency pass closing a nine-agent reference + e2e-standards sweep: rebuilt the staleness dashboard from actual frontmatter; corrected the `commands/index.md` canonical-vs-pointer misclassification that had excluded the canonical commands reference from staleness tracking for two releases; purged three stale Zod-schema counts (39 → 35) and re-verified the union counts; recomputed Coverage Metrics.)
...
**Date basis: 2026-07-30.** `Days Stale` = date basis minus the file's own `last_validated` frontmatter **on disk**. Re-derived this session; nothing carried forward.
...
| commands/index.md (CANONICAL) | 0          | 14        | **NEEDS-VALIDATION** | <!-- NEEDS-VALIDATION added 2026-07-30 (reconciliation pass). The date is current and the dashboard therefore cannot flag this — the marker is the only signal. GAP: the `uninstall` section documents the corrupt-**global**-config warn during deregistration ("Could not update the global project registry") but NOT the corrupt-**project**-config path added the same day. `loadUninstallConfig` in src/cli/commands/uninstall.tsx now catches ConfigLoadError only, warns `Could not read the project config`, returns null, and lets the uninstall proceed to remove the manifest and exit 0 — previously the rejection escaped `run()` and aborted the command exactly when the config was corrupt, i.e. when the user most needs to uninstall. Source finding: agent-findings/2026-07-30-configloaderror-call-sites-lack-a-declared-posture.md. Prior re-validation 2026-07-30 (product 0.146.0): removed the deleted `uninstall --all` flag (oclif now rejects it) + breaking-change callout; rewrote uninstall/compile/validate sections; corrected `list`; added propagated-project recompile to init+edit; FILTER_INCOMPATIBLE added to the feature-flag inventory; INFO_MESSAGES 6 -> 7. ROW LABEL FIXED 2026-07-30: this row tracks `reference/commands/index.md` (canonical, 631 lines), NOT root `reference/commands.md` (30-line pointer). The old label plus a diagram annotation reading `index.md # -> commands.md (pointer)` left the canonical commands reference outside staleness tracking, which is the mechanical reason it drifted through 0.145.0 and 0.146.0 still advertising `--all`. Prior 2026-04-21 iter25: removed dead `build stack` entry (file does not exist in src/cli/commands/build/, only marketplace.ts + plugins.ts); fixed `search` entry (search.ts not .tsx, query required, zero flags — baseFlags overridden to {}); fixed `doctor` flags (no flags, baseFlags overridden, setVerbose(true) unconditional); added feature-gate notes on `new skill`/`new agent`/`new marketplace` (all three gated behind FEATURE_FLAGS.\*\_COMMAND, currently false in src/cli/lib/feature-flags.ts); added Feature-Gated Commands section --> |
| type-system.md (POINTER)      | 0          | 30        | OK (pointer)         | <!-- COUNTS RE-VERIFIED 2026-07-30 against src/cli/types/generated/source-types.ts: SkillId 222, SkillSlug 222, Category 89, Domain 9, AgentName 23 — all still correct. NOTE: `defaultCategories` (src/cli/lib/configuration/default-categories.ts) now DEFINES all 89 categories (27 exclusive, 6 required), up from 51; the 38 added definitions are pinned to the generated union by default-categories.test.ts with toStrictEqual. This file owns the union counts per the bible's count-ownership registry. Prior — CONVERTED TO POINTER 2026-04-21 iter41: original was 469 lines, heavily drifted (claimed 161 SkillIds / 51 Categories — actually 222 / 89; missing D-231 version removal on ProjectConfig; missing D-217 source field on SkillConfig/SkillReference/Skill; missing SkillConfig + AgentScopeConfig + SkillDefinition entries that exist in types/core-types.md). Content already duplicated in types/core-types.md + types/operations-types.md + types/zod-schemas.md (all iter 23/24/41 validated). Replaced body with pointer + semantic-shift index + verified counts. Rationale: maintaining two parallel views drifts; splits are the source of truth going forward. -->                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| test-infrastructure.md (POINTER) | 7 | 30 | OK (pointer) | <!-- NOT re-stamped 2026-07-30 — deliberate. The sweep validated its four `testing/*` children but made no content judgement on this redirect file, so per the Pointer Freshness Rule its date stays 2026-07-23 (7 days, well inside the 30-day link-integrity threshold). All four redirect targets verified to resolve 2026-07-30. Do NOT churn this to the current date. Prior — iter42 converted to pointer. Body was drifted (2026-04-13, missing dual-scope POM, waitForWizardFooter, BaseStep contract, test-fs-utils/expected-values splits) while testing/* children were current (2026-04-21). Now matches type-system.md pointer pattern. -->
...
| wizard/state-transitions.md (CANONICAL) | 0 | 14 | OK | <!-- ROW LABEL FIXED 2026-07-30: tracks `reference/wizard/state-transitions.md` (canonical, 547 lines); root `reference/state-transitions.md` is a 28-line pointer and is one of the 9 untracked pointers. Re-validated 2026-07-30 (D-277): toggleDomain/guard tables + DOMAINS-step-unreachable-from-edit note. Prior 2026-04-21 iter13: removed stale reference to deleted `src/cli/components/hooks/use-wizard-initialization.ts`; rewrote Hydration section against actual `hydrateWizardStore()` in wizard-store.ts (empty history + direct jump, not step walk); documented that `focusedSkillId` is seeded asynchronously by CategoryGrid mount effect (Scenario B race); added Tombstone Lifecycle Transitions (active-project↔dual-scope↔global-only per D-223/D-224), Diff Projection section (D-230/D-232 pre-filter baseline), and Cancellation/Exit transitions -->
| findings-impact-report.md | 0 | 30 | OK — **REGEN DISCHARGED** | <!-- FULL REGENERATION 2026-07-30 (fourth edit of the day; the regeneration the prior pass recorded as OWED). Both triggers discharged: Incremental held 12 entries (>10) and the 0.145.0+0.146.0 bundle had shipped. Every primary table rebuilt over ALL findings on disk; the prior 121-file basis is RETIRED, so the "do not reconcile two bases" warning that stood here no longer applies — there is one basis. **No count is restated in this row** — the report owns the findings totals per the count-ownership registry and carries the arithmetic in its snapshot-boundary callout; re-derive from there. Incremental Updates is genuinely EMPTY (the 12 promoted entries were deleted, not annotated-in-place, so the >10 trigger cannot mis-fire). Patterns: A..S carried forward with widened STATEMENTS for E (rule collision, not just fixtures), O (not code-specific — prose count duplication is the same mechanism), Q (assertion vocabulary as well as frame signal) and S (8 members); **T promoted** from the "Candidate Pattern T" stub the prior pass staged for this decision (5 instances, not 1); **U newly named** (self-audits structurally unable to detect their target defect class). Also this pass: the findings directory's FIRST link-integrity scan — 7 defects found and fixed (2 dangling targets whose files were deleted contrary to the never-move rule, 3 one-sided supersession pairs, 2 unpaired `superseded_by:`). Status-less count moved 39→36 as a result; `TEMPLATE.md` corrected to "36 of 135". `standards/documentation-bible.md` still says "39 of 125" in two places — OUT OF SCOPE for codex-keeper, reported in the report's new "Cross-surface defects reported, not fixed" section. Prior — FULL REGENERATION 2026-07-30 over 121 files, patterns lettered A..S, Incremental reset. -->
...
| testing/e2e-infrastructure.md | 0 | 14 | **NEEDS-VALIDATION** | <!-- NEEDS-VALIDATION added 2026-07-30 (reconciliation pass). Date is current, so the dashboard cannot flag it — this marker is the only signal. THREE verified count/name drifts from the same day's test-coverage batch, each re-counted on disk: (1) the doc enumerates `STEP_TEXT` exhaustively as "All 64 members" — disk now has **65**; the missing member is `UNINSTALL_CONFIG_UNREADABLE` ("Could not read the project config"). An exhaustive enumeration that is one short is worse than a glob, because it reads as authoritative. (2) `e2e/commands` spec total claimed **30**, disk has **31** (`uninstall-corrupt-config.e2e.test.ts`, 5 specs). `e2e/interactive` 47 and `e2e/lifecycle` 75 re-verified CORRECT. (3) the test-utils helper table omits `writeCorruptConfig(baseDir, source)`, added beside writeProjectConfig/writeConfigTypes as their error-path sibling. ALSO PENDING (from agent-findings/2026-07-30-symlinked-project-path-bugs-are-unreachable-from-e2e.md): one sentence under "Scope & HOME model" recording that `process.cwd()` is `getcwd(2)` and returns the kernel-canonical path, so the harness can choose WHICH directory a command runs in but never by WHICH PATH it is reached — without it, an agent can still ship a symlink E2E spec that cannot fail. Prior — 2026-07-24 D-226/D-219 harness update: added launchInProject/launchInGlobal + globalHome getter + globalHome? reuse option to InitWizard/EditWizard inventories; corrected runCLI + CLI.run HOME behaviour (sibling ai-e2e-home-* default, precedence env>globalHome>dir); added "Scope & HOME model" section cross-linking anti-patterns "Choosing the Wizard Launcher by Scope"; removed stale HOME:cwd claim; verified against e2e/ source. Prior — re-validated 2026-04-21 iter35: deeper pass on non-keypress content. Fixed file counts (interactive 32→33, lifecycle 25→45); added missing files (info-panel-scope-toggle-diff, edit-plugin-hard-error, edit-wizard-dual-scope-indicator, plugin-install-failure-hard-error, tombstone-cleanup-PtoG-restoration, ~20 scope-toggle/lifecycle adds); added `assertions/` dir entry; fixed split note (search-interactive doesn't exist; removed build-stack reference); expanded Constants table (added WIZARD_TRANSITION=45s, PLUGIN_TEST=90s, INTERNAL_RETRIES, full EXIT_CODES + SOURCE_PATHS); added new E2E Helpers section (CLI_ROOT, BIN_RUN, FORKED_FROM_METADATA, runCLI, createTempDir, delay, stripAnsi, ensureBinaryExists, writeProjectConfig, createLocalSkill, createPermissionsFile, addForkedFromMetadata, injectMarketplaceIntoConfig, getEjectedTemplatePath, readMarketplaceJson, agentsPath/skillsPath, HOME isolation pattern); added Test-Lifecycle Patterns section (beforeAll/afterAll/afterEach); clarified vitest.config.ts (no setupFiles, no poolOptions); tightened expected-values.ts (9-skill tuple, WEB_AND_API getter). Previous: iter 9 added keypress rule. -->
...
| types/zod-schemas.md | 0 | 14 | OK | <!-- re-validated 2026-07-30: this doc OWNS the Zod schema count per the bible's count-ownership registry — the number is deliberately NOT restated here. COUNT CORRECTION: the previous annotation asserted "confirmed 39 exported schemas match code". That was wrong from 2026-07-23 onward; a direct count of `export const *Schema` in src/cli/lib/schemas.ts gives 35 (4 bridge / 8 loader / 16 structural / 7 strict). schemas.ts grew ~63 lines across 0.145.0–0.146.0 without adding a schema (the growth is splitMetadataValidationIssues and its helpers), so 35 is both the current and the previously correct value. See agent-findings/2026-07-30-doc-index-pins-counts-that-only-the-indexed-doc-revalidates.md. Prior 2026-04-21 iter24: D-231 cross-ref on projectConfigLoaderSchema (version field removed); fixed utility-function drift (formatZodErrors → formatZodIssues, added isCustomMetadata) -->
...
| config/config-writer.md | 0 | 14 | OK | <!-- ANNOTATION CORRECTED 2026-07-30 (reconciliation pass): the previous annotation read "D-277 self-heal" and predated two sections the file now carries. Verified against source this session: (1) NEW **"Path normalization — `normalizeProjectPath`"** subsection under `projects` Field Lifecycle — documents the module-private `fs.realpathSync` helper in local-installer.ts and its THREE call sites (registerProjectPath, deregisterProjectPath, and the current-project skip in propagateGlobalChangesToProjects), the deliberate absence of a `path.resolve` fallback tier (a second tier would be the multi-tier resolution chain CLAUDE.md bans), and a where-the-throw-lands table per caller. (2) The adjacent callout flipped from "Normalization asymmetry (open)" to **"CLOSED — Do not reintroduce it"**, kept rather than deleted because the constraint is invisible in the fixed code. (3) "Cross-Scope Reconciliation — reconcileProjectSplitAgainstGlobal (D-279)". Closes Priority Action 3 in findings-impact-report.md. NOTE the file's cross-reference to `2026-04-21-d233-projects-normalization-asymmetry.md` was updated this session too — that finding is now `superseded`, not `partial`. Prior — iter8 added "Registration observability" (stale-filter drops, user-visible-signal table, cross-link to the registerProjectPath sweep finding); iter7 added "Propagation observability". -->
...
**Date basis:** 2026-07-30. Every row re-derived from disk frontmatter this session.

**Derivation (all 41 `reference/**/\*.md` files, counted on disk 2026-07-30):\*\*

- **37 files** carry `last_validated: 2026-07-30`. **4 files** carry `2026-07-23`.
- The 4 un-restamped files are all **pointers**: `test-infrastructure.md` (tracked, row above), `wizard/component-patterns.md`, `wizard/flow.md`, `wizard/store-map.md` (untracked). Two agents deliberately declined to re-stamp them because they made **no content judgement** on them. That is correct behaviour and is now codified — see the Pointer Freshness Rule.
- **Result: 31 of 32 tracked rows are 0 days stale.** The single non-zero row is `test-infrastructure.md` at **7 days**, inside its 30-day pointer threshold. No row is `DUE` or `OVERDUE`.
- **Re-verified unchanged by the 2026-07-30 reconciliation pass.** That pass re-extracted all 41 frontmatter dates and re-stamped **nothing**: the two docs it found content drift in were flagged, not re-dated. Re-stamping a file a pass did not content-validate would have destroyed the only signal distinguishing "verified" from "swept past" — the Pointer Freshness Rule's consequence 1, applied to non-pointers.

> **`NEEDS-VALIDATION` is orthogonal to `Days Stale` — read both columns.** Two rows
> (`testing/e2e-infrastructure.md`, `commands/index.md`) are flagged `NEEDS-VALIDATION` **while
> reading 0 days stale**, and that is not a contradiction. `Days Stale` measures the age of the
> file's `last_validated` stamp; the flag records that source has changed _since_ that stamp, within
> the same day. A doc validated at 09:00 and invalidated by a 17:00 code change is simultaneously
> 0 days stale and wrong. **A pass that consults only the staleness dashboard will miss every
> same-day drift**, which is exactly the class the 2026-07-30 bug-fix batch produced.
...
The 9 untracked pointers, **verified by reading each file** rather than inferred from path depth:

| Untracked pointer                  | Redirects to                  | Disk `last_validated` |
| ---------------------------------- | ----------------------------- | --------------------- |
| `architecture/overview.md`         | `architecture-overview.md`    | 2026-07-30            |
| `architecture/dependency-graph.md` | `dependency-graph.md`         | 2026-07-30            |
| `architecture/boundary-map.md`     | `boundary-map.md`             | 2026-07-30            |
| `commands.md`                      | `commands/index.md`           | 2026-07-30            |
| `state-transitions.md`             | `wizard/state-transitions.md` | 2026-07-30            |
| `config/configuration.md`          | `features/configuration.md`   | 2026-07-30            |
| `wizard/flow.md`                   | `features/wizard-flow.md`     | 2026-07-23            |
| `wizard/store-map.md`              | `store-map.md`                | 2026-07-23            |
| `wizard/component-patterns.md`     | `component-patterns.md`       | 2026-07-23            |
...
| --------------------- | ------ | ------------------------------------------------------------------------------------------------------ | ------------ | -------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Architecture Overview | [DONE] | `reference/architecture-overview.md`                                                                   | 2026-07-30   | 2026-07-30     | D-277: isInitMode no longer bypasses the global-item guard. Validate in 30 days                                                                                                                                                                                                                                                                                                                                                               |
| Commands Reference    | [DONE] | `reference/commands/index.md` (canonical; `reference/commands.md` is now a pointer)                    | 2026-07-30   | 2026-07-30     | Validate in 14 days                                                                                                                                                                                                                                                                                                                                                                                                                           |
| Type System           | [DONE] | `reference/type-system.md`                                                                             | 2026-07-30   | 2026-07-30     | Validate in 14 days                                                                                                                                                                                                                                                                                                                                                                                                                           |
| State Management      | [DONE] | `reference/store-map.md`                                                                               | 2026-07-30   | 2026-07-30     | D-277: applySkillRemoval ownership semantics, applyAgentToggle 3-arg signature, toggleDomain view filter, preselectAgentsFromDomains merge. Validate in 7 days                                                                                                                                                                                                                                                                                |
| Compilation Pipeline  | [DONE] | `reference/features/compilation-pipeline.md`                                                           | 2026-07-30   | 2026-07-30     | Validate in 14 days                                                                                                                                                                                                                                                                                                                                                                                                                           |
| Configuration System  | [DONE] | `reference/features/configuration.md`                                                                  | 2026-07-30   | 2026-07-30     | Validate in 14 days                                                                                                                                                                                                                                                                                                                                                                                                                           |
| Wizard Flow           | [DONE] | `reference/features/wizard-flow.md`                                                                    | 2026-07-30   | 2026-07-30     | D-277: four init-mode bypass sentences corrected. Validate in 14 days                                                                                                                                                                                                                                                                                                                                                                         |
| Skills & Matrix       | [DONE] | `reference/features/skills-and-matrix.md`                                                              | 2026-07-30   | 2026-07-30     | Validate in 14 days                                                                                                                                                                                                                                                                                                                                                                                                                           |
| Plugin System         | [DONE] | `reference/features/plugin-system.md`                                                                  | 2026-07-30   | 2026-07-30     | Validate in 14 days                                                                                                                                                                                                                                                                                                                                                                                                                           |
| Component Patterns    | [DONE] | `reference/component-patterns.md`                                                                      | 2026-07-30   | 2026-07-30     | D-277: init-mode diff suppression annotated vestigial. Validate in 14 days                                                                                                                                                                                                                                                                                                                                                                    |
| Utilities Reference   | [DONE] | `reference/utilities.md`                                                                               | 2026-07-30   | 2026-07-30     | Validate in 14 days                                                                                                                                                                                                                                                                                                                                                                                                                           |
| Test Infrastructure   | [DONE] | `reference/test-infrastructure.md` (POINTER -> `testing/*`)                                            | 2026-07-23   | 2026-07-23     | POINTER — intentionally not re-stamped 2026-07-30; all 4 redirect targets verified to resolve. 7 days / 30-day link-integrity threshold. Do not churn                                                                                                                                                                                                                                                                                         |
| Operations Layer      | [DONE] | `reference/features/operations-layer.md`                                                               | 2026-07-30   | 2026-07-30     | Validate in 14 days                                                                                                                                                                                                                                                                                                                                                                                                                           |
| Agent System          | [DONE] | `reference/features/agent-system.md`                                                                   | 2026-07-30   | 2026-07-30     | D-277: retainReconciled*→retainProjectOwned* name drift fixed; agent-side self-heal documented. Validate in 14 days                                                                                                                                                                                                                                                                                                                           |
| Dependency Graph      | [DONE] | `reference/dependency-graph.md`                                                                        | 2026-07-30   | 2026-07-30     | Validate in 14 days                                                                                                                                                                                                                                                                                                                                                                                                                           |
| Boundary Map          | [DONE] | `reference/boundary-map.md`                                                                            | 2026-07-30   | 2026-07-30     | Validate in 14 days                                                                                                                                                                                                                                                                                                                                                                                                                           |
| State Transitions     | [DONE] | `reference/wizard/state-transitions.md` (canonical; `reference/state-transitions.md` is now a pointer) | 2026-07-30   | 2026-07-30     | D-277: toggleDomain/guard tables + DOMAINS-step-unreachable-from-edit note. Validate in 14 days                                                                                                                                                                                                                                                                                                                                               |
| Findings Impact       | [DONE] | `reference/findings-impact-report.md`                                                                  | 2026-07-30   | 2026-07-30     | **FULL REGENERATION NOW OWED** — the >10-entry Incremental trigger was crossed by the 2026-07-30 reconciliation append, and the 0.145.0+0.146.0 release-bundle trigger is live too. Both were recorded rather than actioned (that pass was scoped not to rebuild primary tables). Rollups stay pinned to their snapshot until then. **Finding counts are NOT restated in this map** — that file owns them; read its snapshot-boundary callout |

### New Files (Phase 2+3 restructure, 2026-04-13)

| Area                    | Status | File                                      | Last Updated | Last Validated | Next Action         |
| ----------------------- | ------ | ----------------------------------------- | ------------ | -------------- | ------------------- |
| Test Infrastructure     | [DONE] | `reference/testing/infrastructure.md`     | 2026-07-30   | 2026-07-30     | Validate in 14 days |
| Test Factories          | [DONE] | `reference/testing/factories.md`          | 2026-07-30   | 2026-07-30     | Validate in 14 days |
| Test Mock Data          | [DONE] | `reference/testing/mock-data.md`          | 2026-07-30   | 2026-07-30     | Validate in 14 days |
| E2E Infrastructure      | [DONE] | `reference/testing/e2e-infrastructure.md` | 2026-07-30   | 2026-07-30     | Validate in 14 days |
| Core Types              | [DONE] | `reference/types/core-types.md`           | 2026-07-30   | 2026-07-30     | Validate in 14 days |
| Operations Types        | [DONE] | `reference/types/operations-types.md`     | 2026-07-30   | 2026-07-30     | Validate in 14 days |
| Zod Schemas             | [DONE] | `reference/types/zod-schemas.md`          | 2026-07-30   | 2026-07-30     | Validate in 14 days |
| Scope System            | [DONE] | `reference/concepts/scope-system.md`      | 2026-07-30   | 2026-07-30     | D-277 guard rows    |
| Tombstone Pattern       | [DONE] | `reference/concepts/tombstone-pattern.md` | 2026-07-30   | 2026-07-30     | D-277 provenance    |
| Guard Pattern           | [DONE] | `reference/concepts/guard-pattern.md`     | 2026-07-30   | 2026-07-30     | D-277 preconditions |
| Edit Command (Detailed) | [DONE] | `reference/commands/edit.md`              | 2026-07-30   | 2026-07-30     | Validate in 14 days |
| Config Writer (Detail)  | [DONE] | `reference/config/config-writer.md`       | 2026-07-30   | 2026-07-30     | D-277 self-heal     |
| Config Merger Contract  | [DONE] | `reference/config/config-merger.md`       | 2026-07-30   | 2026-07-30     | D-277 absence note  |
| Config Scope Split      | [DONE] | `reference/config/scope-split.md`         | 2026-07-30   | 2026-07-30     | D-277 provenance    |
...
| E2E Testing Bible             | `standards/e2e-testing-bible.md`      | 2026-03-25 | **2026-07-30** | Rewritten in the 2026-07-30 sweep (inverted `HOME=cwd` claims removed, counts re-derived)                                   |
| E2E Sub-Standards             | `standards/e2e/` (7 files)            | 2026-03-25 | **2026-07-30** | All 7 files re-stamped 2026-07-30: README, anti-patterns, assertions, page-objects, patterns, test-data, test-structure     |
...
| Documentation Standards       | `standards/documentation-bible.md`    | 2026-03-25 | **2026-07-30** | Count-ownership rule, Pointer Freshness Rule, Heading Diff rule, rebuilt hook table, Known-Limitations re-validation clause |
...
### ESLint: gate closed 2026-07-30 — runnable, with a known non-zero baseline

**This section previously said ESLint did not exist and instructed agents not to run it. That is no
longer true.** ESLint was adopted on 2026-07-30 at the user's explicit request.
...
- **`npm run lint` currently exits 1 with 150 problems.** That baseline is pre-existing code, not a
  broken setup. Breakdown: 108 `@typescript-eslint/no-unused-vars` (across `src/cli/lib`, `e2e/`,
  `src/cli/commands`, `src/cli/components`), 38 `@typescript-eslint/no-explicit-any` (all in
  `scripts/generate-source-types.test.ts`), plus 4 one-offs.
- **Lint the files you touched, not the whole repo** (`npx eslint <paths>`), until the baseline is
  burned down. Do not report "no ESLint errors" for a repo-wide run — say what your files did.
- **Do NOT run `eslint --fix` across the repo.** Only 2 of the 150 are auto-fixable and both fixes
  are wrong — see `agent-findings/2026-07-30-eslint-disable-directives-were-never-verified.md`. That
  is why the `lint-staged` entry is check-only.
- `prettier --check .` also currently fails, on 55 pre-existing `.ai-docs/**/*.md` files. Unrelated
  to ESLint, but noted here because `prepublishOnly` runs it first and will stop there.
...
resolved`). Two older findings proposed lint rules that now have somewhere to live —
`2026-07-20-e2e-spec-files-accumulate-unused-imports-unenforced.md` is covered (`no-unused-vars` now
runs over `e2e/`); `2026-07-17-d167-task-id-recurrence-no-lint-guard.md` is **not** — it needs a
`no-restricted-syntax` rule that was deliberately left out to keep the initial rule set stock.
...
**Source Files:** **347** TypeScript files in `src/cli/` — **183** production (excludes `*.test.*`, `__tests__/`, `__mocks__/`) + **127** test specs (`*.test.*`) + **35** non-spec `__tests__/` support files + **2** `__mocks__/`. (The `__tests__/` directories hold 79 files total: 44 specs + 35 support.) Re-counted on disk 2026-07-30 (reconciliation pass) — **unchanged** from the index-consistency pass earlier the same day. The day's source fixes edited existing files and added no new production module; the one new source-side test (`local-installer.test.ts` symlink case) went into an existing spec file.
**E2E Suite:** **197** TypeScript files in `e2e/` (**160** `*.test.ts(x)` + **37** helpers/fixtures/pages) — re-counted on disk 2026-07-30 (reconciliation pass; was 196 / 159 / 37 earlier the same day). The +1 spec is `e2e/commands/uninstall-corrupt-config.e2e.test.ts` (5 tests). Support count is unchanged: `writeCorruptConfig` and `STEP_TEXT.UNINSTALL_CONFIG_UNREADABLE` were added to **existing** files (`e2e/helpers/test-utils.ts`, `e2e/pages/constants.ts`), which is why the file total moved but the support total did not.
**Agent Findings:** count NOT restated here — owned by `reference/findings-impact-report.md` per the count-ownership registry in `standards/documentation-bible.md`. That file's snapshot-boundary callout carries both the pinned-snapshot basis and the current disk total, and is the only place either number is written. Re-derive from it, never from this map.
**Agent Suggestions:** no proposals awaiting a decision. The two added 2026-07-30 were both adopted the same day — `2026-07-30-identity-key-helper-export-exception.md` (`absorbed`; export exception in `CLAUDE.md`, key-families rule landed as **8.7**) and `2026-07-30-column-geometry-snapshot-rule-6-17a.md` (`absorbed`; rule 6.17a in `standards/clean-code-standards.md`). Named rather than counted, so this line cannot drift.
**Product Version:** 0.146.0 (`package.json`, verified 2026-07-30)
...
4. **type-system.md Zod Schemas**: PASS at the time -- schema count confirmed, 5 random schemas spot-checked (`domainSchema`, `boundSkillSchema`, `metadataValidationSchema`, `hooksRecordSchema`, `skillFrontmatterLoaderSchema`). <!-- 2026-07-30 correction: the count recorded here was 39 and is now 35; per-schema line numbers removed (line numbers in docs are banned by project convention). The count is not restated here — `reference/types/zod-schemas.md` owns it. -->
...
- Zod schema count: verified against `schemas.ts` on this entry's date <!-- 2026-07-30: the phrase "39 exported schemas (still correct)" was removed. It read as a present-tense claim; the count is 35 as of 2026-07-30 and this doc does not own it. Owner: reference/types/zod-schemas.md. -->
