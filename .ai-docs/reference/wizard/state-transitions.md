...
| `toggleAgent()`              | `GLOBAL_AGENTS_LOCKED` toast if agent is installed globally, or if spacebar hits a live dual-scope `[P][G]` row (inert — only `s` changes it); both gated on not-in-global-edit and not-in-init mode.                                                       |
