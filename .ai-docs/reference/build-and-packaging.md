...
There is **no CI publish workflow.** `.github/workflows/` holds exactly one file
(`notify-web.yml`, a cross-repo dispatch when the catalog moves). Publication is a manual
`npm publish`, and `prepublishOnly` is the only gate between the working tree and the registry.

> **`prepublishOnly` currently fails at its first step.** `npx prettier --check .` exits `1` on the
> working tree — one offender, `src/cli/lib/seed/fetch-seed.ts`. `.ai-docs/**/*.md` is clean
> (`prettier --check ".ai-docs/**/*.md"` exits `0`), which retires the older "prettier fails on
> pre-existing `.ai-docs` markdown" note still recorded under "Known Tooling Gaps" in
...
9. **`prepublishOnly` stops at `format:check`, before anything else runs.** A publish attempt today
   fails on `src/cli/lib/seed/fetch-seed.ts` and never reaches lint, typecheck, build or test (§1).
