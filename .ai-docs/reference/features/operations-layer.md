...
**Key Files:** 21 production (2 root + 3 subdomain barrels + 16 operation files), 10 co-located test files
...
| `src/cli/lib/operations/types.ts`                             | Convenience type-only re-exports — NON-exhaustive subset of `index.ts`; omits `MarketplaceRequirement`, `CompileAllScopesOptions`, `CopyLocalSkillsOptions` (import those from `index.js`) | root      |
...
| `detectBothInstallations` | `(projectDir: string) => Promise<BothInstallations>`               | `detectGlobalInstallation()`, `detectProjectInstallation()`, `isHomeDirectory()` from `installation/index.js`                                                                                                                                                                    | Checks global and project independently; skips project when projectDir is home                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
- **Internal cross-references** -- Operations may call other operations in the same subdomain (e.g., `compareSkillsWithSource` calls `collectScopedSkillDirs`; `requireMarketplace` calls `ensureMarketplace`; `compileAgentsAllScopes` calls `compileAgents`).
