...
<!-- PARTIAL 2026-08-02 · config-gate landing (`last_validated` deliberately NOT moved)
     ✓ writeProjectConfig row + its lower-level dependency row, the init/edit/compile
       command-flow blocks, the init/edit Command Consumers rows, the D-228/D-240/D-279
       Related Findings rows, the lib→operations convention — re-derived from
       src/cli/lib/config-gate/ and src/cli/commands/{init,edit,compile,uninstall}
     ✗ every other operation row — still on the 2026-07-30 basis
-->
...
| `recompilePropagatedProjectAgents` | `(projectDirs: string[]) => Promise<PropagatedRecompileSummary>`   | `recompileRegisteredProjectAgents()` (internal), `getErrorMessage()` from `utils/errors.js`                                                                                                                                                                       | D-240: sequential loop with per-project failure isolation. A thrown error or a non-empty `result.failed` increments `failedCount` and pushes a warning; neither aborts the loop. Sequential so warnings keep a deterministic per-project order. Called only by `config-gate/recompile.ts` (lazily, to avoid the lib → operations cycle) — no command calls it directly since 2026-08-02. Commands `warn()` each entry of `GateReport.recompile.warnings` and log the recompiled/failed counts                                                                                                                                                                                                                                                                     |
...
init.run()
  ...
  |           project-config write — D-279; classification decides the fan-out)
  |          -> propagateGlobalChangesToProjects() -> recompilePropagated()  // D-240
...
| D-228            | `writeProjectConfig`                                                                   | Project `config-types.ts` must be written via `regenerateConfigTypes` (import-from-global), never the standalone renderer. Structurally enforced since 2026-08-02: the standalone renderer is private to `config-gate/pair-writer.ts` and `regenerateConfigTypes` throws at `$HOME`.                                  |
...
| D-240            | `config-gate/`, `recompileRegisteredProjectAgents`, `recompilePropagatedProjectAgents` | A write that propagates RECOMPILES the propagated projects' agents itself and returns a `GateReport`; callers render the report. Per-project failures are isolated, never fatal. Closes `2026-07-18-propagation-skips-agent-recompile.md`.                                                                            |
...
| D-279            | `writeProjectConfig`                                                                   | `reconcileProjectSplitAgainstGlobal` runs immediately before BOTH project-config writes (`writeScopedFromWizard`'s project branch and `propagateGlobalChangesToProjects`), each of which then calls the shared `writeProjectConfigPair`. See `2026-07-29-project-config-written-by-two-paths-only-one-reconciled.md`. |
