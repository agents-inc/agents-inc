...
e2e/
  ...
  lifecycle/                         # Lifecycle E2E tests — 70 files (grew substantially during tombstone + scope-toggle + stack-cleanup work)
    ...
    dual-scope-agent-badge-and-s-inert.e2e.test.ts
    ...
    dual-scope-spacebar-collapse-live-selection.e2e.test.ts
    dual-scope-spacebar-reselect-restore.e2e.test.ts
...
Wraps a `TerminalSession` plus its cleanup dirs; exposes the ready `StackStep` as `readonly stack`. Options: `InitWizardOptions` = `{ source?, projectDir?, cols?, rows?, env?, noSource?, skipPermissions?, loadTimeout?, defaultTimeout? }`.

| Method                                           | Returns            | Purpose                                                                                                               |
| ------------------------------------------------ | ------------------ | --------------------------------------------------------------------------------------------------------------------- |
| `static launch(options?)`                        | `InitWizard`       | Launch `init [--source <dir>]`, then `StackStep.waitForReady(loadTimeout)`.                                           |
| `static launchRaw(options?)`                     | `InitWizard`       | Launch and poll for non-empty output WITHOUT waiting for the stack step — for resize-warning / pre-stack asserts.     |
...
Exposes the ready `BuildStep` as `readonly build` — edit opens directly to the build step (no stack step). Options: `EditWizardOptions` = `{ projectDir (required), source?, cols?, rows?, env?, extraArgs?, defaultTimeout? }`.

| Method                           | Returns        | Purpose                                                                                                                                                                  |
| -------------------------------- | -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `static launch(options)`         | `EditWizard`   | Launch `edit [--source <dir>] [...extraArgs]`; three-sentinel wait `BUILD_FOOTER` → `waitForWizardFooter` → `BUILD` (all `TIMEOUTS.WIZARD_TRANSITION`) before returning. |
