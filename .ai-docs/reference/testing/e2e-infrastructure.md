...
**Count ownership.** `STEP_TEXT`'s member count is written in exactly two places — the row above and the grouped table in [standards/e2e/README.md § Constants Quick-Reference](../../standards/e2e/README.md). They have drifted apart twice already (64 here vs 72 there vs a true 74). Whichever doc a validation pass re-derives the count in, grep the other for the old value in the same session.
