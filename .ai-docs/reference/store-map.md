...
| `_sessionRebuiltScopePairSkills` | `Set<SkillId>`               | Skill IDs whose dual-scope `[P][G]` pair was (re)built by a store action THIS session (via `s`/`toggleSkillScope` G→P re-adding the tombstone, or a spacebar re-select in `toggleTechnology`). The persisted-pair guard in `toggleSkillScope` consults this set so `s` round-trips a session-rebuilt pair instead of locking it. Internal (leading underscore); not for UI. Reset to `new Set()` by `createInitialState`.                                                              |
| `_sessionRebuiltScopePairAgents` | `Set<AgentName>`             | Agent-name equivalent of `_sessionRebuiltScopePairSkills`, consumed by the persisted-pair guard in `toggleAgentScope` and the restore path in `toggleAgent` (`restoreDualScopeAgent`). Reset to `new Set()` by `createInitialState`.                                                                                                                                                                                                                                                   |
...
| `toggleAgent`           | `(agent: AgentName) => void`                        | Add/remove agent. Dual-scope aware (D-233): deselecting a `[P][G]` pair collapses it to inherited-global via `collapseDualScopeAgent`; re-selecting an inherited-global row rebuilds the pair via `restoreDualScopeAgent` (records id in `_sessionRebuiltScopePairAgents`). Guards against toggling globally-installed agents from project scope (`GLOBAL_AGENTS_LOCKED` toast); tombstone-aware via `applyAgentToggle`. |
...
| Action                            | Signature                                    | Effect                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| --------------------------------- | -------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `toggleSkillScope`                | `(skillId: SkillId) => void`                 | Toggle skill scope project/global. No-op if `isEditingFromGlobalScope`. Guards: (1) a persisted dual-scope `[P][G]` pair (tombstone in `installedSkillConfigs`, not yet in `_sessionRebuiltScopePairSkills`) is a guarded no-op → `INSTALLED_AT_BOTH_SCOPES` toast; (2) project eject→global blocked when a global eject is already installed → `ALREADY_EJECTED_AT_GLOBAL` toast (unless an excluded tombstone allows the undo). G→P adds an excluded global tombstone (if previously installed) and records the id in `_sessionRebuiltScopePairSkills`; P→G drops any global tombstone. |
...
| `toggleAgentScope`                | `(agentName: AgentName) => void`             | Toggle agent scope project/global. No-op if `isEditingFromGlobalScope`. Mirrors `toggleSkillScope`: a persisted dual-scope pair (tombstone in `installedAgentConfigs`, not in `_sessionRebuiltScopePairAgents`) is a guarded no-op → `INSTALLED_AT_BOTH_SCOPES` toast; G→P adds the excluded global tombstone and records the id in `_sessionRebuiltScopePairAgents`; P→G drops it.                                                                                                                                                                                                       |
...
| `preselectAgentsFromDomains` | `() => void`                                                  | After domain selection: replace `selectedAgents`/`agentConfigs` from `DOMAIN_AGENTS`, preserving excluded tombstones (D-227)                                                                                                                         |
...
**`applySkillRemoval(configs, removedIds, installedSkillConfigs)`**: Removes project-scope skills outright; marks global-scope skills as `excluded: true` only if they appear in `installedSkillConfigs` (previously installed on disk). A dual-scope pair collapses to a single inherited-global entry (D-233 Scenario B). Invoked by `toggleDomain`, `toggleFilterIncompatible`, and `reconcileSkillConfigs`.

**`reconcileSkillConfigs(configs, added, removed, installedSkillConfigs, isEditingFromGlobalScope)`**: Wrapper applied by `toggleTechnology`. Calls `applySkillRemoval` for removed skills (passing `null` for installed when `isEditingFromGlobalScope`, so a removal is a genuine uninstall — D-233 Scenario C), then for each added skill: rebuilds the `[P][G]` pair on a dual-scope restore, un-excludes an existing tombstone, or appends `buildSkillConfigForId(id, installedSkillConfigs)`.
...
**Agent config builders/togglers:** `buildAgentConfigForName(name, savedConfigs?)` mirrors `buildSkillConfigForId` on the agent path (preselection). `applyAgentToggle(configs, agent, isSelected, installedAgentConfigs)` mirrors the skill tombstone logic. `collapseDualScopeAgent(...)` / `restoreDualScopeAgent(...)` are the D-233 dual-scope collapse/restore used by `toggleAgent`. `collectTombstones(configs)` returns excluded entries (preserved across reconcile/preselect merges — D-223/D-227), used by `populateFromSkillIds`, `preselectAgentsFromDomains`, `preselectAgentsFromStack`.
...
| `INSTALLED_AT_BOTH_SCOPES`  | `Installed at both scopes — use space to change project scope` | `toggleSkillScope`, `toggleAgentScope` (persisted pair) |
| `ALREADY_EJECTED_AT_GLOBAL` | `Already exists as ejected skill at global scope`              | `toggleSkillScope` (project eject → global)             |
| `GLOBAL_AGENTS_LOCKED`      | `Global agents cannot be changed from project scope`           | `toggleAgent`                                           |
...
- `unresolvableSkillIds: []`, `_sessionRebuiltScopePairSkills: new Set()`, `_sessionRebuiltScopePairAgents: new Set()`
