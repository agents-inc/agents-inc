...
`finalConfig` whose `projects` field is now `undefined`. The written global
`config.ts` loses its `"projects": [...]` entry. The subsequent propagation
guard `if (finalConfig.projects?.length)` at line 843 returns falsy, so
`propagateGlobalChangesToProjects` never runs.
