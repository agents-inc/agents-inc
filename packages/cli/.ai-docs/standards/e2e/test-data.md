...
- **Never fake an error state with unrealistic content when a realistic setup produces the same state.** Measured across `e2e/` when the renderer was tightened: of 99 `renderMetadataYaml(` call sites, **81 across 34 files** omitted at least one field `localRawMetadataSchema` requires — `displayName`, `slug`, `category` and `domain` — including two `ProjectBuilder` factories, `minimal()` and `editable()`, which between them stand behind the compile, edit and lifecycle suites. No product path writes that file — the scaffolder that seeds a new marketplace skill (`metadataYaml` in `src/cli/lib/marketplace-scaffold.ts`) emits all four and more, and an ejected skill is copied whole from a source carrying the same — so the fixtures were the drift and the schema was not. Six specs across four files turned out to DEPEND on the gap, having made a skill "unresolvable" by installing it with metadata the loader could not use; once the metadata described the skill, the wizard resolved it and the specs had nothing left to assert. They now reach that state the way a project genuinely reaches it — config entries with no files, via `unresolvableSkills` on `ProjectBuilder.editable` / `pluginProject`. Filling the four fields by default healed 82 of the then-100 sites without touching them.
...
```
grep -rnE '^\s*(domain|slug|category|displayName|contentHash|author): [^,;{]*[^,;{[:space:]]\s*$' e2e --include='*.ts'
```
...
**Use `setupIsolatedHome(prefix)`** from `src/cli/lib/__tests__/helpers/isolated-home.ts`. It creates the temp dir, chdirs into a project inside it, points `HOME` at a _sibling_ directory — never the project, or global and project state collapse into one place — and returns the `cleanup` that restores both. `useFakeHome(getTempDir)` is the hook-registering sibling for files that already own their temp dir.

**`process.env.HOME` alone is not enough where the code path calls `os.homedir()`.** Node re-reads `$HOME` on every call, so a mutated env var is picked up; **bun resolves it once at startup and ignores later mutation**, and this package runs its tests under both. Any spec reaching `loadGlobalSourceConfig`, `isHomeDirectory` or the global local-skill merge needs `vi.spyOn(os, "homedir").mockReturnValue(fakeHome)` beside the env var. `build:marketplace`'s catalog spec and the `resolveBranding` block in `config.test.ts` both carry the pair.
