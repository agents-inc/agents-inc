...
**Instead:** the comment states which count drives the bound, which domain that count was measured against, and what running out looks like — so the number is a grep target for the next taxonomy edit rather than a bare literal. `SOURCE_ROW_WALK_LENGTH` (`e2e/pages/steps/sources-step.ts`) is the surviving worked example: it names the fixture's ten skills, states that `classifySkillSourceRows` emits at most one focusable row per skill, and says the value is "ten plus headroom" — an upper bound derived from something, not a round number. It is an example of the COMMENT, not a claim that the bound is right: a row is open against it for being fixture-sized, so a larger source under-walks and passes vacuously. That is the point rather than a caveat to it — the comment is what let anyone notice.

**`MAX_FOCUS_ATTEMPTS` was this section's other example and no longer exists** (deleted 2026-08-20). Read what happened to it before reaching for a well-commented bound, because it is the harder half of this rule: raised to 50 and re-commented exactly as prescribed above — "against web's 33 since the taxonomy split" — it went on to produce a red CI build anyway (run 32338714325). The comment was accurate and the number was still wrong, because `focusSkill` pressed Tab BEFORE reading the screen, so a target in the category the grid opens on cost a full 33-press lap, and a press assumed to have landed let the walk skip past its own target — needing a second lap that 50 could not fund. A bound documented to this standard survives a taxonomy edit; it does not survive being the wrong shape.

What replaced it terminates on the structure being walked rather than on a count: the walk stops when it observes a category it has already seen — one real lap, every category looked at exactly once — and raises `CategoryWalkError` naming them. **Whether that generalises into a rule of its own is open** — `2026-08-20-focus-walk-presses-before-it-looks.md` proposes it, and a row carries it as ruling-wanted. Until then this section stands as written: if a numeric bound is what the situation gives you, name the count it stands for.
