...
When a documented system has active hardening tasks in `todo/cli.md` (e.g., D-214), the reference doc MUST include a **Known Limitations** section cross-referenced to the task ID with file/function anchors. Describing code as if its TODOs were closed is drift.
