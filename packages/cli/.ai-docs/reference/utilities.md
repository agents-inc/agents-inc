...
| `EDITOR_URL`                 | `https://agentsinc.sh/editor`                                      | The editor — where a configuration is built without the wizard and handed to `init --from <id>`. The editor Worker's Route, `agentsinc.sh/editor*` (`apps/editor/wrangler.jsonc`) — the path segment is part of the address, and the apex above it is the marketing site, which knows nothing about `?fromId=`; its config store `https://api.agentsinc.sh` is spelled separately in `src/cli/lib/seed/fetch-seed.ts` as `SEED_API_URL`, which is env-overridable while this is not |
