...
e2e/
  ...
  commands/                          # Command (non-interactive) E2E tests — 31 files
    ...
    uninstall-corrupt-config.e2e.test.ts               # 0.146.1: uninstall proceeds past an unreadable project config
  ...
  interactive/                       # Interactive wizard E2E tests — 54 files (PTY-based)
    ...
    confirm-step-info-panel-parity.e2e.test.ts         # 0.147.0: confirm step and I overlay render one component
    ...
    init-wizard-sources-added-markers.e2e.test.ts      # 0.147.0: first init flags every row as added on the Sources tab
    init-wizard-stack.e2e.test.ts
    init-wizard-stack-agents.e2e.test.ts
    init-wizard-stack-banner.e2e.test.ts               # 0.147.0: LOGO_MIN_TERMINAL_ROWS gate — see fixture-cardinality caveat below
    ...
    smoke.e2e.test.ts
    sources-focused-row-marker-spacing.e2e.test.ts     # 0.147.0: first spec to assert the FOCUSED marker form head-on
    sources-grid-scope-row-headers.e2e.test.ts         # 0.147.0: Scope caption removed, row headers + gutter kept
    sources-inert-row-selection-check.e2e.test.ts      # 0.147.0: locked / pending-removal rows paint no checkmark
    ...
    wizard-terminal-resize-guard.e2e.test.ts           # 0.147.0: mid-session shrink replaces the wizard, restores on grow
...
- `toHavePluginInRegistry(key, scope?)` - Verify plugin registry. **Not a registry-presence check alone (2026-08-02):** for every installation listed under the key it runs `describeMissingContent(installation, pluginId)` and FAILS when the entry has no content on disk, naming what is missing. A registry row pointing at nothing is the exact "artefact that looks like verification and cannot fail" shape, so do not weaken this back to a JSON-key lookup.
...
| `resizeBelowMinimum(cols, rows)`     | **Added 0.147.0.** Snapshot raw cursor → `session.resize(cols, rows)` → `waitForTextAfter(STEP_TEXT.RESIZE_PROMPT, cursor)`. Closed-loop on the resize prompt.                                                                                                                                                                              |
| `resizeAboveMinimum(cols, rows)`     | **Added 0.147.0.** Snapshot raw cursor → `session.resize(cols, rows)` → `waitForWizardFooterAfter(cursor)`. Anchored on the footer emitted AFTER the resize, not on the copy already sitting in scrollback from before the shrink.                                                                                                          |
...
- Suite size at the time of the estimate was ~65 E2E files (164 `*.e2e.test.ts` as of 0.147.1, re-counted on disk) × ~5-10 keypresses per test × ~100ms worst-case = ~30-60s total added under contention. Under no contention: negligible (first-poll hits).
