...
Re-derive both the counts and the membership from source rather than carrying either forward. The
barrels (`factories/index.ts`, `helpers/index.ts`, `assertions/index.ts`) re-export from siblings, so
`scripts/check-enumeration-drift.ts` cannot bind these three tables — its source reader names a
module's OWN declarations and reads no `export … from`. The single-file inventories further down
(FS utilities, expected values, content generators, the journey-page reader) ARE registered there.
