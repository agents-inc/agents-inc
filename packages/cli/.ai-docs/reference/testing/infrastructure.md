...
Which DIRECTORIES hold specs is a separate question, and it is STATED in `SPEC_DIRECTORIES` rather
than read off the tree. Derived, a directory's kind was a fact about whatever happened to be sitting
in it: `e2e/fixtures/` counted as helper-land only because no spec had ever been written there, so
the first one put there turned a journey row's `fixtures/dual-scope-helpers.initGlobalWithEject` —
a module and a symbol inside it — into a spec reference answering to no file. `classify()` performs
that lookup and nothing else consults the tree for it. The hand-kept half is answered by a roster
gate rather than by care: `journey-page.test.ts` holds the constant against
`specDirectoriesIn(readSpecNames(E2E_ROOT))` with `toStrictEqual`, so a tree that gains or loses a
spec directory reddens one named assertion saying which directory and in which direction.
`specDirectoriesIn` is exported for that gate alone — **the reader must never consult it**, because a
reader measuring its own vocabulary off the tree is the coupling the stated constant exists to cut,
and a gate comparing a derivation against itself agrees whatever the tree does. The
