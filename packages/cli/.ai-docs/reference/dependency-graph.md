...
<!-- PARTIAL 2026-08-02 (b) · post-landing reconciliation (`last_validated` deliberately NOT moved)
     Closes the gap the pass below names in its own ✗: it covered edit / compile / eject /
     uninstall / new * and NOT init or update, and both were missing edges.
     ✓ every `src/cli/commands/**` file's lib/ imports re-derived by enumerating them from
       source and DIFFING against the rows (not by checking the rows) — init += config-gate
       (GateReport type) + lib/seed/; update += operations recompilePropagatedProjectAgents,
       config-gate normalizeProjectPath, configuration/project-config; eject += lib/loading/.
       Notes 14a + 14b added.
     ✗ the Operations -> Lib, Components and utils tables — still on the bases below
-->
<!-- PARTIAL 2026-08-02 · config-gate landing (`last_validated` deliberately NOT moved)
     ✓ every edge into/out of the new `lib/config-gate/` module: the command rows for
       edit / compile / eject / uninstall / new *, the writeProjectConfig and
       recompile-project-agents operation rows, and notes 8, 13, 14 — re-derived by
       grepping `config-gate` imports across src/cli
     ✗ every other edge — still on the 2026-08-01 FULL basis below
-->
...
| `update`            | `commands/update.tsx`           | `loadSource`, `compareSkillsWithSource`, `compileAgents`, `collectScopedSkillDirs`, `findSkillMatch`, `discoverInstalledSkills`, **`recompilePropagatedProjectAgents`** (2026-08-02 — `update` now refreshes every registered project's agents, not just the cwd's)                                                      |
...
| `uninstall`         | `commands/uninstall.tsx`        | `loadAgentDefs` (agent defs for the global-uninstall project-pruning pass, D-274)                                                                                                                                                                                                                                        |
...
| `update`            | `lib/skills/` (injectForkedFromMetadata, SkillComparisonResult type), `lib/loading/source-loader` (**SourceLoadResult** type), **`lib/config-gate/`** (normalizeProjectPath), **`lib/configuration/project-config`** (loadProjectConfigFromDir) — both added 2026-08-02 by the registered-project refresh                                                                                                                                                                                                                                                                                                                                                                                      |
...
| `hooks/use-category-grid-input.ts`      | `lib/feature-flags`                | `FEATURE_FLAGS` (gates the `F` filter-incompatible keypress, D-269)       |
...
13. **The uninstall command reaches deepest into `lib/config-gate/`** -- `propagateGlobalRemoval` (global) + `mutateGlobal({ kind: "deregister-project" })` (project) + `lazyGateDeps`, plus `isHomeDirectory` / `resolveInstallPaths` from `lib/installation/` and `loadSkillsMatrixFromSource` from `lib/loading/`. There is no `uninstallProject` operation; the removal plan, the plugin teardown and the registered-project pruning all live in the command (D-274). Since 2026-08-02 the pruning also recompiles the pruned projects' agents — inside the gate, reported back through `GateReport.recompile`.

14. **`commands/compile.ts` now depends on `lib/loading/` and `lib/config-gate/reconcileTypesFromDisk`** -- previously compile touched neither. This is the `config-types.ts` refresh pass; it uses `matrixOnly: true` so the `lib/loading/` edge does not make compile network-dependent. The gate edge also makes `compile` at `$HOME` a propagating command: it fans the hand-edited global config out to every registered project and recompiles their agents.

14a. **`commands/update.tsx` became a fan-out command on 2026-08-02, and its edges did not follow.** It now imports `recompilePropagatedProjectAgents` from `lib/operations/`, `normalizeProjectPath` from `lib/config-gate/` and `loadProjectConfigFromDir` from `lib/configuration/project-config` — none of which this file recorded. The fan-out is the CONTENT twin of the gate's CONFIG fan-out: `update` rewrites the skill directories every registered project compiled its agents from, so it reuses the gate's own recompile helper. It deliberately does **not** go through config-gate and writes no pair — a skill's content changes nothing any config declares — which is why `normalizeProjectPath` had to be exported from the gate as a pure matcher (it writes nothing, so exporting it hands out no privilege). Note 16 below is the reason this was missed: the row was checked, not diffed.

14b. **`commands/init.tsx` is the sole consumer of `lib/seed/`**, importing `fetchSeedConfig` (`fetch-seed`) and `seedToWizardResult` (`seed-to-wizard`). The whole `lib/seed/` module was absent from this file until 2026-08-02. The contract itself is now documented: [features/seed-contract.md](./features/seed-contract.md) (wire format, version-discard policy, the payload -> `WizardResultV2` mapping, and the `--from` consumer path).
