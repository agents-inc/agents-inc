...
**Anti-pattern:** Lib modules MUST NOT statically import the operations layer (operations sit above lib and import back into it, so a static edge forms a load-time cycle that corrupts Vitest module mocks). When a lib function must reuse an operations helper it uses a lazy `await import(...)` inside the function body -- `config-gate/recompile.ts` imports `recompile-project-agents` (the whole reason that file exists), and both `config-gate/index.ts`'s `lazyGateDeps` and `configuration/config-types-writer.ts`'s `loadConfigTypesDataInBackground` import `operations/project/load-agent-defs`. `configuration/config-types-writer.ts` and `config-gate/index.ts` use the same technique for `loading/source-loader`; that one is lib->lib, deferred for cost rather than to break a cycle. Re-derive the census rather than carrying it -- `grep -rn 'await import(' src/cli/lib --include='*.ts' --include='*.tsx'`, excluding specs.
...
| `share`             | **`lib/seed/`** (seedPayloadForInstallation from `installation-payload`, publishSeedConfig from `publish-seed`) — and nothing else. It mints an id for what is installed here and reports it; `edit --ui` mints the same id from the same two calls and opens it instead                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| `edit`      | `components/wizard/run-wizard-session` (runWizardSession), `components/wizard/wizard` (WizardResultV2 type), `components/common/spinner` (Spinner), `components/common/removal-plan-confirm` (**RemovalPlanConfirm**, RemovalPlanSection type), `components/common/prompt-confirm` (promptConfirm)                         |
...
| Utils      | `utils/open-url.ts` — the one utils→utils edge; it spawns the platform's link opener through `execCommand`                              |
...
| Lib      | 44             | 41 under `lib/` plus 3 in `lib/operations/`: compiler, loading/_, configuration/_, installation/_, skills/_, plugins/_, stacks/_, agents/\*, resolver, versioning, schema-validator, source-validator, permission-checker, marketplace-generator |

Total: 50 production consumers (most-used utility in the project)
...
| Lib        | 42             | 38 under `lib/` plus 4 in `lib/operations/`: compiler, loading/_, configuration/_, installation/_, skills/_, plugins/_, stacks/_, agents/_, matrix/_, schemas, output-validator, versioning |

Total: 51 production consumers
...
| Components | 0              | —                                                                                                                                                                                             |
| Operations | 4              | project/recompile-project-agents.ts, skills/install-plugin-skills.ts, skills/uninstall-plugin-skills.ts, source/ensure-marketplace.ts                                                         |
| Utils      | 1              | open-url.ts                                                                                                                                                                                   |
| Lib        | 21             | compiler, config-gate/propagate, content-validator, loading/_, configuration/_ (incl. project-config), installation/_, skills/_, plugins/_, stacks/_, agents/\*, versioning, source-validator |
...
| Layer    | Consumer Files                                                                      |
| -------- | ----------------------------------------------------------------------------------- |
| Commands | `base-command`, `init`, `edit`, `compile`, `uninstall`, `update`, `search`, `share` |
| Lib      | `lib/seed/installation-payload.ts`, `lib/loading/source-fetcher.ts`                 |

Total: 10 consumers. **Not the command layer alone.** Two lib modules print through it: the seed producer reuses `ERROR_MESSAGES.NO_INSTALLATION` so `share` and `edit --ui` refuse in the same words every other command does, and the source fetcher owns the revalidation notice that has to go out before the download starts rather than after.
...
| Stores     | 1              | wizard-store.ts                                                                                                                                                                                |
| Lib        | 24             | 23 under `lib/` plus `operations/skills/discover-skills.ts`: compiler, matrix/_, loading/_, configuration/_, installation/_, stacks/_, agents/_, plugins/\*, resolver, wizard/build-step-logic |

Total: 31 production consumers
...
Total: 1 production consumer. A leaf over a leaf: `edit` -> `utils/open-url` -> `utils/exec`. No dependency is added for it — the three platform openers are shell-outs to something the OS already ships, and the URL travels as its own argv entry with no shell, so the argument vector is the whole injection guard.
...
| Lib        | `agents/list-compiled-agents.ts`, `configuration/config-generator.ts`, `configuration/config-writer.ts`, `loading/source-fetcher.ts`, `matrix/matrix-health-check.ts`, `plugins/plugin-validator.ts`, `schemas.ts`, `source-validator.ts`, `stacks/stacks-loader.ts` |

Total: 11 production consumers
...
1. **`init` and `edit` are the heaviest commands.** `init` takes 6 operations plus 6 direct lib modules; `edit` takes 9 operations plus 10 direct lib modules. They orchestrate the full install/edit pipeline, and `edit` additionally owns both directions of the shared-configuration round trip.
