...
**Category definitions (separate file, separate count).** `defaultCategories` in `src/cli/lib/configuration/default-categories.ts` must define a member for every `Category`. Any member it omits is auto-synthesized at load time with a humanized name, `order: 999` and `exclusive: false` — which is why the file must stay exhaustive. Pinned against the generated union by `src/cli/lib/configuration/__tests__/default-categories.test.ts`. Its size and the `exclusive: true` count are owned by [`features/skills-and-matrix.md`](./features/skills-and-matrix.md) ("Current Counts").
