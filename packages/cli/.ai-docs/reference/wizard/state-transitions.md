...
**Both of those calls are wrapped in `showWarningsAsToast(...)` at the call site**, because either can `warn()` about a skill the wizard cannot place and this is the first transition past the mount, where `warn()` writes to a stderr the next repaint scrolls away. The wrapper is at the call site rather than inside the actions precisely because `hydrateForInit` reaches both of them BEFORE the mount, where buffering is already open and the startup band is the right surface. See [`../store-map.md`](../store-map.md) → "Mid-Session Warnings".
