...
| --ui               | boolean | Open agentsinc.sh/editor instead of the terminal — on whatever `--from` names, or on the catalogue when it names nothing. Opens the editor on NOTHING — there is no installation here to carry across, which is what separates it from `edit --ui`. Prints the address before opening one, so it stays usable over a pipe, in CI and where no desktop session exists. Reads no installation at all, above `ensureConfigReadable`, so a config too broken to load cannot block the other front door |
...
- `dashboardCountLines(data: DashboardData): string[]` -- the skill/agent/mode/marketplace rows, rendered by BOTH dashboard paths so the two cannot diverge
- `formatDashboardText(data: DashboardData): string` -- the non-TTY summary, composed from `dashboardCountLines`
- `Dashboard` component -- the TTY dashboard; exported for `init.test.tsx`, which holds its frame against `dashboardCountLines`
...
| Flag            | Type    | Description                                                                                                                                                                                                                                    |
| --------------- | ------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| --ui            | boolean | `default: false`. Open agentsinc.sh/editor instead of the wizard, on whatever `--from` names. Given no `--from`, that is THIS installation, so an id is minted for it first; given one, the id already exists and nothing is minted or fetched |
...
6. For each pass (`runCompilePass`):
   a. `seatMatrixForPass(projectDir)` -- **first, before anything in the pass reads the matrix.** Returns `MergedSkillsMatrix | null`, held for step h. See "The matrix is seated once per pass" below.
   b. `discoverAllSkills()` -- **Operation: `discoverInstalledSkills(projectDir)`**
   c. If `totalSkillCount === 0`: log `No skills found for <label> pass, skipping`, still run `refreshConfigTypes(params, cwd, seatedMatrix)`, and return `false`. The config loads independently of discovered skills, so a hand-edited `config.ts` listing skills with nothing installed for that scope must still get fresh unions.
   d. `warnUnresolvedStackSkills()` -- emits `this.warn()` for each configured stack skill absent from disk (excluded ids filtered via `effectivelyExcludedSkillIds`); such skills are dropped from the recompiled agents rather than silently omitted
   e. `warnScopeDroppedStackPairs()`
   f. **Operation: `compileAgents({ projectDir, sourcePath, skills, pluginDir, outputDir, scopeFilter })`**
   g. When the pass compiled zero agents and `label === "Project"`, `hintGlobalScopedAgents()` counts the config's active `scope === "global"` agents and, if non-zero, prints `globalScopedAgentsHint(count)` after `INFO_MESSAGES.NO_AGENTS_TO_RECOMPILE`
   h. `refreshConfigTypes(params, cwd, seatedMatrix)`
...
**`config-types.ts` regeneration.** The documented workflow is to hand-edit `config.ts` then run `compile`, so every pass regenerates the type unions for the scope it compiled via `reconcileTypesFromDisk(projectDir, config, { matrix: seatedMatrix, agents }, { currentProjectDir: cwd })` (`src/cli/lib/config-gate/index.ts`), matching the wizard write path exactly: standalone narrowed unions at global scope, import-and-extend at project scope (`regenerateConfigTypes`). The hand-edited `config.ts` is an input and is never rewritten. Success logs `INFO_MESSAGES.CONFIG_TYPES_REFRESHED`. When `loadProjectConfigFromDir` finds no config the refresh is skipped at verbose level. **Any failure downgrades to a warning** (`configTypesRefreshFailed(reason)`) -- the compiled agents are already written and remain valid; only the unions may be stale.

**A `null` seat skips the refresh whole.** `refreshConfigTypes` returns before loading the config when `seatedMatrix` is `null`, warning `configTypesRefreshFailed("no skills catalogue could be loaded for <projectDir>")`. It does NOT write `config-types.ts` from `BUILT_IN_MATRIX`, and at global scope it therefore fans nothing out. Every union it emits is derived from the catalogue, so writing them without one is a wrong answer rather than a degraded one -- the unions would narrow to the built-in catalogue's categories while the `config.ts` beside them still keys its `stack` under the ones that vanished, and at global scope that mismatch would be propagated into every registered project.
...
**The matrix is seated once per pass, and the catalogue is a VALUE the pass carries.** `seatMatrixForPass(projectDir)` calls `loadSkillsMatrixFromSource({ projectDir, skipExtraSources: true, matrixOnly: true })` as the pass's first act. That load seats the module-level `matrix` singleton in `src/cli/lib/matrix/matrix-provider.ts` AND the helper returns the catalogue it seated; `runCompilePass` holds it and hands it to `refreshConfigTypes`, which passes it to `reconcileTypesFromDisk` rather than reading the singleton back. A refresh reading the singleton cannot say which seat it got, which is the whole reason the value is threaded. **The two readers are why the seat comes first, and they take opposite postures on a failure.** The render path reads the singleton through `statedUsageFor` and `liveCategoryOf` (`src/cli/lib/stacks/stacks-loader.ts`), so no argument reaches it; an unseated pass leaves it at its process-start `BUILT_IN_MATRIX` default, which carries no local skill — a locally-installed skill's stated `usageGuidance` then falls back to the generic per-category sentence, writing different bytes than `install` last wrote and making the next `compile` rewrite every agent carrying one. The scope is `pass.projectDir`, never the invoking `cwd`, so a global pass seats the global installation's own local skills. On a failure the helper warns `Failed to load skills matrix for <projectDir>: <reason>` and answers `null`: the RENDER degrades to that placeholder behaviour, and the type refresh ABORTS rather than writing unions off a catalogue nobody loaded.

`matrixOnly` skips the `fetchFromSource` clone for the default source (the matrix is the pre-computed `BUILT_IN_MATRIX` anyway) so `compile` stays offline on a cold cache. `skipExtraSources` is not a divergence from the wizard's fully tagged load -- extra sources only annotate `availableSources`/`activeSource` for wizard UI tagging and neither the render path nor the config-types writer reads them, so the emitted output is byte-identical (pinned by the `skipExtraSources` parity test in `local-installer.test.ts`).
...
**Args:** none. **Flags:** `--stdin` (boolean) — the ONE input that is not the installation in the cwd.
...
1. The payload — `payloadFromPipe()` under `--stdin`, `payloadFromInstallation()` otherwise. Two producers, one publish: the flag decides WHERE the configuration comes from and nothing after it differs.
   1a. `seedPayloadForInstallation(process.cwd())` (`src/cli/lib/seed/installation-payload.ts`) — the read, the ownership judgement, the content reading, the mapping and the empty guard, all five in one call. `{ ok: false, error }` -> `this.error(prepared.error, { exit: EXIT_CODES.ERROR })`. **`edit --ui` calls the same function**, so the two commands mint the same id from the same directory and differ only in the ending.
...
4. `reportShared(id)` — `Shared as <id>`, then both lines of `sharedConfigDestinations(id)`: `Install it:  npx agents-inc init --from <id>` and `Open it:     https://agentsinc.sh/editor/?fromId=<id>`. An id nobody can act on is not a share, and exactly two things read one.
...
- `updateRegisteredProjects()` (after removal) calls `propagateGlobalRemoval(globalConfig, { matrix, agents })`, which reaches `pruneGlobalEntriesFromRegisteredProjects(globalConfig, agents)` -- stripping the inlined global-scoped entries from every registered project and regenerating each project's `config-types.ts`. It runs **after** the global manifest is gone so the regenerated project types fall back to the standalone form instead of importing from a deleted global `config-types.ts`. Each `report.propagated.skipped` path warns via `registeredProjectUpdateSkipped(path)`; a non-empty `report.propagated.updated` logs `registeredProjectsUpdated(count)` and then renders `GateReport.recompile` through `reportPropagatedRecompile`. A throw is caught and warned -- **no failure here may abort the uninstall**.

**The captured `matrix` reaches no reader on this path.** `propagateGlobalRemoval` takes the pair as a `LoadedGateDeps` and passes `deps.agents` alone to the prune; every catalogue lookup in the fan-out is against the per-project seat `withCatalogueSeatedFor` installs. `prepareGlobalPropagation` still loads it, and `matrixOnly` + `skipExtraSources` keep that load offline and quiet: an uninstall must not hang on a cold cache or fail on an unreachable remote.
