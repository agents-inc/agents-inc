...
- `run-wizard-session.tsx` - `runWizardSession(options: WizardSessionOptions): Promise<WizardResultV2 | null>`: the single shared command-side entry point that hydrates the store, renders `<Wizard>`, awaits exit, clears the terminal (Ink `clear()` then the command-owned `clearTerminal`), and returns the result — `null` both when the render produced no result and when the result carries `cancelled: true` (used by `init.tsx` and `edit.tsx` only). It captures `onComplete` internally into a closure variable and forwards `options.onCancel`; callers supply everything else via `WizardSessionOptions = { hydrate: HydrateOptions; props: Omit<WizardProps, "onComplete" | "onCancel">; onCancel: () => void; clearTerminal: () => void }`. `list.tsx` does NOT use it — it renders a read-only `ListView` (via `SkillAgentSummary`) and calls `hydrateWizardStore()` directly to seed the diff baseline, without rendering the wizard.
  ...
  **Hydration runs inside a second logger buffering window.** `hydrateIntoStartupBand(hydrate, loaded)` in the same file wraps `hydrateWizardStore()` in `enableBuffering()` / `drainBuffer()` / `disableBuffering()` and concatenates the drained messages onto the load's own, so the array handed to `<Wizard startupMessages>` is `[...loaded, ...drained]`. Hydration WARNS — `populateFromSkillIds` reaches `resolveSkillForPopulation` in `stores/wizard-store.ts`, which warns once for an installed skill the loaded source no longer carries and once for one whose category no domain claims — and the load's own buffer is already drained by then (in `lib/operations/source/load-source.ts`), so without this window those lines are a plain stderr write under a frame that is about to be painted at full terminal height. They would be pushed off the top of the screen the moment it paints.
...
### Two shapes `computeScopeDiff` has to produce deliberately

The Global section renders at most one row per skill, and a tombstone is a mask rather than an install. Neither property falls out of the natural reading of the baseline, so each is produced by a named rule — and this document owns the statement of both.

| Shape                                                                      | Renders             | The rule that produces it                                                                                                                                                                                                                                                                                                                   |
| -------------------------------------------------------------------------- | ------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Snapshot `[G active]` + live `[P active]`, no tombstone (inherited global) | One `unchanged` row | Both routes by which a baseline global entry can re-surface are deduped against `inheritedSkillIdSet`: `uniqueExcludedGlobalSkills` drops the current-tombstone route, `removedGlobalSkills` drops the removal route. Without the second the entry is admitted as inherited AND matched as removed, and one skill lands under Global twice. |
| Snapshot `[G tombstone]` + live `[]`                                       | Nothing             | Removal reads `activeSkillBaseline` — `installedSkillConfigs.filter((s) => !s.excluded)` — never the raw snapshot, so a baseline tombstone is never a removal candidate. Its slot held nothing to delete, and a `-` there would announce a deletion that does not happen.                                                                   |

Agents are handled the same way through `inheritedAgentNameSet` and `activeAgentBaseline`. The Sources tab reaches both answers by its own route: `isSlotAlreadyRendered` in `wizard-store.ts` suppresses the duplicate, and snapshot tombstones are excluded there as removal candidates.
