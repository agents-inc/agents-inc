...
**Directory:** `src/cli/lib/seed/` — eight modules, no barrel. Consumers import
...
| `src/cli/lib/seed/read-piped-payload.ts`                 | `readPipedPayload`, `PipedPayload`, `NOTHING_PIPED`, `STDIN_IS_A_TERMINAL` — everything `share --stdin` can refuse locally, refused BEFORE the POST. Three mistakes told apart: nothing arrived, what arrived is not JSON, or it is JSON `installableSeedPayloadSchema` will not take — the WRITE schema, deliberately not the base one `fetch-seed.ts` reads with, so the scope-reach rule is refused here rather than at the edge as a bare 400. The schema rather than a shape written here is the point of publishing from the CLI at all — `SEED_VERSION` is a `z.literal`, so a producer that hardcodes the wire format emits refused payloads the day it moves. |
...
graph TD
  ...
  Fetch --> Schema["@workspace/matrix/seed<br/>the wire contract"]
...
| `src/cli/lib/seed/config-to-seed.ts`                | `SEED_VERSION`, `installableSeedPayloadSchema`, `seedModelSchema`                                       | Mints a payload, and `parse`s rather than merely assembles it — with the INSTALLABLE schema, because this module only ever mints                                                                                                                                                                        |
...
| `src/cli/lib/seed/config-to-seed.test.ts`           | `SEED_VERSION`, `installableSeedPayloadSchema`                                                          | Asserts the minted envelope is one the contract's own schema accepts                                                                                                                                                                                                                                    |
...
| `apps/editor/src/lib/api/configs.ts` -> `ShareRefusal`   | Turns the status into `out-of-date`, one of four refusals the editor tells apart. Reported to Sentry under its own name, since a stale-tab count rises after a release and decays on its own                                                                             |
| `src/cli/lib/seed/publish-seed.ts` -> `refusalMessage`   | Branches on `CONTRACT_MISMATCH` (409) ahead of any body read and answers `OUT_OF_DATE_AGAINST_STORE` — the CLI's own words, because the worker's body names a reload and there is no page in a terminal. See [`publish-seed.ts`](#publish-seedts--the-outbound-boundary) |

**Only the POST.** `GET /configs/:id` runs no version negotiation at all: what it serves was
validated on the way in and is immutable under a content-addressed id, so a stored payload that no
longer parses is an integrity failure of the worker's own (500). Its handler `getConfig` declares
404, 500 and 503 and no 409. A version this CLI cannot read therefore never reaches the read side as
a status — it lands in `fetchSeedConfig`'s schema-failure message alongside every other way a stored
payload stops parsing, and re-sharing is the one remedy true of all of them. See
[Failure is a message, never a throw](#failure-is-a-message-never-a-throw).
...
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Writing** — `POST /configs`, and every client that mints a payload for it or forwards one: `config-to-seed.ts`, `read-piped-payload.ts`, `apps/editor/src/lib/api/configs.ts` | `installableSeedPayloadSchema` | A link that cannot be installed should never get an address. 400 rather than a stored id — except a wrong `v`, which is [409](#a-wrong-v-on-the-write-side-is-409-not-400). A client that gates on the READ schema instead passes exactly what the edge refuses, and the user sees the 400 with no reason attached. |
...
**The editor gates at the write BOUNDARY, not at the mint, and the two are different places on
purpose.** Its mint is `toSeedPayload` in `apps/editor/src/features/configure/lib/seed.ts`, which
`parse`s with the base `seedPayloadSchema`; the gate is `writeContractProblems` inside
`createSharedConfig` in `apps/editor/src/lib/api/configs.ts`. Tightening the mint would be wrong
because the mint also serves the output-preview dialog and local Save, where a configuration
carrying an unwritable pair has to round-trip untouched — opening one and repairing it in a click is
what the lenient read schema exists for. So the stricter half applies at the one moment a payload
becomes a WRITE. Every POST the editor makes crosses that function — Share (`use-share-link.ts`), the
install dialog (`use-install-command.ts`), a signed-in Save (`roster-panel.tsx`) and the local slot a
first sign-in adopts (`stores/account-store.ts`) — which is what stops a fifth arriving unprotected.

It asks the SCHEMA rather than `unwritableSeedAssignments`, so a rule added to the write contract
later is enforced there without anyone remembering to come back. A refusal returns the `unwritable`
member of `ShareRefusal` — the one refusal decided locally rather than read off a response, and the
only one whose fix is already on screen. `scope-reach.spec.ts` -> "a signed-in save of the pair"
holds that the refusal precedes the POST rather than merely producing the same words after one: it
reads the request log as well as the button, because a `createSharedConfig` that posted first and
mapped the worker's 400 back would put identical words on that button, and a refusal costing a round
trip is the defect the pre-POST gate removed rather than a friendlier spelling of it. It asserts the
SECOND write is unspent too — a signed-in save is a mint and then a stored pointer, so an early
return dropped from the handler would file a row whose `configId` is the empty string.

**A refusal at the SIGN-IN seam is not a no-op, and the fourth POST is where that matters.**
`adoptLocalStack` in `apps/editor/src/stores/account-store.ts` carries the browser's local snapshot
into an account on a first sign-in, and adoption is a mint and then a write — so it ends at whichever
refused, and both refusal sets meet in `AdoptionRefusal = ShareRefusal | StackRefusal`. What comes
back is a VALUE rather than a dialog, because this runs on a path nobody asked for at the moment they
finish signing in: the store keeps `unadopted`, the grid keeps drawing the local slot beside the
account's list, and `stack-grid.tsx` renders `WHY_NOT_ADOPTED[refusal]` above the grid. Silence was
defensible only while every ending here was transient — a payload the write contract refuses is
refused identically forever, so the snapshot left the grid on sign-in with nothing saying why.

Two consequences follow and both are the accepted outcome rather than cases the store rules out.
**Both lists at once is reachable** — the local slot and the account's rows are told apart by their
second line, since the slot lists the skills it holds where an account's row can only say it is in
the account. And **`unadopted` is a fact about the SNAPSHOT rather than about the account's list**:
only what actually reaches the snapshot may clear it, which is adoption carrying it in or a sign-out
handing the grid back to it. A `save` that cleared it on every save kept a tidier invariant and
dropped a refused slot off the grid one save later. A non-empty account is still asked
`writeContractRefusal` (declared beside `adoptLocalStack` in that same store) — a `safeParse`
against `installableSeedPayloadSchema` needing no request at all — because "we did not try" and "it
is in there" are different answers, and a payload the contract refuses could not have reached that
account by any route.

**The two doubles that MINT validate with the same schema**, so neither is more permissive than the
worker it stands in for: `createConfig` in `packages/api-mocks/src/handlers.ts`, the route's default
resting handler, and `isInstallable` in `e2e/fixtures/seed-config-store.ts`, which decides whether
that store files a posted body at all. A body the write schema refuses is answered 400 carrying
`@hono/zod-validator`'s own `c.json(result, 400)` envelope rather than minting an id — which is what
`publish-seed.ts`'s `refusalSchema` reads back on the wire, and what makes a suite green only where
the real edge would also accept the payload. Handlers installed per test with
`configMockServer.use(...)` stand in for one specific refusal and validate nothing, by design.
...
| --------------------------------------------- | ------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Network unreachable / DNS                     | `try` around the request                   | `Could not reach ${SEED_API_URL} — check your connection.`                                                                                                                                                                |
| Unknown id                                    | `response.status === NO_SUCH_CONFIG` (404) | `No configuration found for id '${id}'.`                                                                                                                                                                                  |
| Any other non-2xx, store's own words readable | `!response.ok` -> `refusalMessage`         | `Fetching configuration failed (HTTP ${status}). The store said: ${explanation}`                                                                                                                                          |
| Any other non-2xx, nothing quotable           | `!response.ok` -> `refusalMessage`         | `Fetching configuration failed (HTTP ${status}).`                                                                                                                                                                         |
...
**A non-2xx carries the store's own account of it wherever there is one to have.** The status is
never given up — it is what separates a store that is present and holding bytes it can no longer read
(500) from one that would not answer at all (503) — and what the body adds is the store saying which
of its own failures this was, in words the status has no room for. `refusalMessage` assembles the
pair, `refusalBody` reads it, `arrivedAsText` decides whether it is the store speaking, and
`explanationOf` trims it and clips it with `truncateText` to `EXPLANATION_BUDGET`, which is `120`
here. Sized on what this route really writes, which is one short line per refusal (`Stored config is
unreadable`, `Could not read this config`); it is deliberately far short of the `300` the constant of
the same name holds in `src/cli/lib/seed/publish-seed.ts`, because each number measures one route
rather than stating a policy about terminals.

**404 is the one status held out of that arm**, ahead of it under `NO_SUCH_CONFIG`. The store answers
it `No config under this id`, which says strictly less than the CLI's own sentence does — it does not
name the id the caller typed, and that is the whole of what a person needs to see.

**The discriminator is the wire's CONTENT TYPE, not an envelope shape**, and that is the difference
from the write side rather than an inconsistency with it. `getConfig` writes every refusal on this
route with `c.text(...)`, so `QUOTABLE_TYPE` (`text/plain`) is the shape the store's own words take
here; a body arriving as anything else came from a proxy, a WAF, a captive portal or a gateway
answering in the store's place, and their answer is markup, which quoted into a terminal buries the
status that does explain. `publish-seed.ts` can match a SHAPE — `refusalSchema` over the
zod-validator envelope — because its refusals are JSON. A sentence has no shape, so the wire's own
statement of what it sent is the only discriminator this side has. **It is not proof**: a hostile
answer may claim any content type it likes, which is what the budget bounds and why what survives is
attributed to the store (`The store said: …`) rather than spoken in the CLI's own voice. Reading a
remote body back into the terminal is a trust-boundary fact — see
[`reference/boundary-map.md`](../boundary-map.md) § 6.5.

**The schema-failure message names the remedy, and deliberately diagnoses no cause.** The payload was
validated by the worker on the way _in_ (`createConfigRoute` parses with
`installableSeedPayloadSchema` before hashing — this schema plus one rule, so anything the POST
accepted satisfied this one when it was stored), so a stored payload that no longer parses means the
contract moved underneath it — but
...
| `claimingACatalogueId` | **Throws** naming every carried id EITHER catalogue already holds — `heldByEitherCatalogue` is `heldByLoadedCatalogue(matrix.skills[id]) \|\| CATALOGUE_SKILL_IDS.has(id)`, where the second set is `typedKeys(BUILT_IN_MATRIX.skills)`. A skill id is the directory it installs into, so a carried skill taking a catalogue id writes its own bytes over that catalogue's copy, and every sub-agent naming the id afterwards is handed whatever the payload shipped. **The two halves answer what the other cannot, and only one of them is unforgeable.** `sharedConfigSourceFlags` in `commands/init.tsx` passes `payload.marketplace` to the loader, so a payload naming a marketplace that does not ship the id it impersonates meets no incumbent in the LOADED half at all; the SHIPPED half is the part no payload can steer. The loaded half in turn catches a collision one marketplace along, with ids the shipped set has never heard of. `heldByLoadedCatalogue` carries the carve-out: an incumbent with `local: true` is not a collision on this axis — the local-skill merge marks a carried skill a previous apply wrote that way — so `edit --from` can re-apply a configuration it already installed. `catalogueIdCollisionError` lists every id untruncated, where the marketplace refusal in `loading/source-loader.ts` stops at `MAX_REPORTED_COLLISIONS`: that one reports a whole repository's directory scan, these are the skills one sharer added by hand to one configuration |
...
**Both of `registerExternalSkills`' refusals are decided before anything is seated**, because the seat
writes into `matrix` in place. A guard placed after it would stop a run that had already replaced
entries every later read in the process resolves, and a guard throwing on the first collision it met
would have overwritten whatever it seated on the way there. So `registerExternalSkills` computes
`askedForAsPlugin` and `claimingACatalogueId` over the whole selection, throws on either, and only
then maps `seatExternalSkill`. The two refusals answer in that order, and the collision one names its
ids because the fix is the sharer's — the id is what has to be re-minted.

**A THIRD refusal lives in `writeExternalSkills`, and it judges the disk rather than the catalogue.**
`refuseUncarriedDestinations` runs before the first byte is written and throws
`uncarriedDestinationError` naming every destination that already holds content no shared
configuration put there — `id  ->  skillDir` per row, with both ways out offered, because only the
user knows whose skill matters more. `judgeDestination` decides one destination: a directory that is
not there is nothing to protect, and a directory whose `metadata.yaml` carries `forkedFrom.path` is
one a shared configuration wrote. Everything else is somebody else's.

**The matrix cannot answer this question, which is why the guard is on the disk side.**
`heldByLoadedCatalogue` carves out every `local` incumbent so a re-apply works, and the local-skill
merge writes `local` on everything it merges — so that carve-out also exempts a skill the user wrote
by hand and a marketplace skill ejected here, neither of which is the round trip's to replace. Worse,
a directory with no readable `metadata.yaml` never reaches the matrix at all, so no in-memory guard
could see it with or without a provenance field on `ResolvedSkill`. `forkedFrom.path` is the
discriminator rather than `forkedFrom` itself: `registerSkillOnDisk` is the only writer of that field
— `copySkillTo` deliberately omits it, because a marketplace skill is installed again by its id — and
`readCarriedSkill` reads it for this same question one layer up. Every destination is judged before
any is written, for the same reason the two refusals above are decided before anything is seated.
...
Past that guard, `writeExternalSkills` does three separate acts per install, and each is separately
load-bearing:
...
`defaultUsageGuidance(install.skill.categoryId)` — `Use when working with <category>.`, with the
CATEGORY and a full stop. Neither default is invented, and they exist because `doctor` validates
every installed `metadata.yaml`: a file this command writes that that command reports as an error is
the CLI disagreeing with itself, in a file the user cannot fix since the skill is somebody else's
repository.

**It is the same CALL as the stack loader's, not the same words.** `defaultUsageGuidance` is
exported from `src/cli/lib/stacks/stacks-loader.ts` for this one reason. A carried skill's cue is
reachable by two routes — this file writes it into the installed `metadata.yaml`, and
`statedUsageFor` answers for it during the run that seats the skill, before any metadata exists — so
two spellings meant one sub-agent could read a different sentence for the same skill depending on
which command last touched it. The editor's preview
(`apps/editor/src/features/configure/lib/output-preview.ts`) mirrors the sentence deliberately
...
| A carried skill claiming a catalogue id | `this.handleError` on the same throw -> `EXIT_CODES.ERROR`, naming every such id and telling the sharer to add each skill again, which mints an `external-<category>-<name>` id no catalogue owns. Nothing is seated, nothing is written, at either scope                            |
...
default the decoder would have to guess back. The return value is
`installableSeedPayloadSchema.parse(...)`d rather than merely assembled — the one gate that catches a
field whose values have drifted out of the wire's enums. The installable half rather than the base
one because this module only ever mints; it refuses nothing the module's own checks permit, and that
is the point, so the day the two disagree the mint fails loudly here instead of minting an id that
cannot be installed.
...
| 409 — the two ends name different contract versions                                      | `Sharing this configuration failed (HTTP 409). ${OUT_OF_DATE_AGAINST_STORE}`         |
| Any other non-2xx carrying the store's refusal envelope (JSON)                           | `Sharing this configuration failed (HTTP ${status}). The store said: ${explanation}` |
| Any other non-2xx whose body is `text/plain` prose the status has not already said (503) | `Sharing this configuration failed (HTTP ${status}). The store said: ${explanation}` |
| Any other non-2xx whose `text/plain` body only restates its own reason phrase (413, 429) | `Sharing this configuration failed (HTTP ${status}).`                                |
| Any other non-2xx with nothing quotable to read                                          | `Sharing this configuration failed (HTTP ${status}).`                                |
| 2xx body is not JSON                                                                     | `The configuration store returned something that is not JSON.`                       |
| 2xx body carries no `id`                                                                 | `The configuration store did not return an id for this share.`                       |

**A refusal carries the store's own account of it wherever there is one to have.** The status is
never given up — it is the only thing separating a bad payload (400) from a stale contract (409), a
body over the cap (413), a rate limit (429) and a store that refused the write (503) — but it cannot
say WHY a payload was bad, and the store already knows: a refused POST answers with the issues its
own schema raised, naming the skill and the sub-agent. So a refused share reads:

```
Sharing this configuration failed (HTTP 400). The store said: skills.<id>.assignments.<agent>: a project-scoped skill has nowhere to be written on '<agent>', which rests at global scope
```

**Reading it back out takes two parses, because the store's `message` is itself a JSON document.**
`apps/server` registers no `defaultHook` and its own hook narrows only the wrong-`v` case, so every
other refused body falls through to `@hono/zod-validator`'s `c.json(result, 400)` — the whole
`safeParse` result, of which a `ZodError` contributes `{ name, message }`. `refusalSchema` takes
that envelope, `refusedIssuesSchema` takes the issues parsed back out of the `message` string, and
`sentenceOf` renders each one path-first, joined with `; `. That rendering is deliberately the same
sentence `formatZodIssue` in `lib/schema-validator.ts` gives a locally-raised issue, and therefore
the same sentence `share --stdin` prints for the pair it refuses before the POST; it cannot call
that function, because an issue read off a wire is a plain object where `formatZodIssue` takes a
`z.ZodIssue` union nothing arriving over HTTP can be narrowed to without a cast.

**Prose is the second arm, and it reaches a terminal by no other route.** `explanationOf` tries the
envelope first — it is the more specific shape, and the order costs nothing either way, since an
envelope arrives as JSON and could never satisfy the prose arm's content-type gate while prose has
no shape for `refusalSchema` to match. `quotableProseIn` then asks three questions in turn:
`arrivedAsText` (the `content-type` starts with `text/plain`, which is how `c.text(...)` writes every
refusal on this route that is not the validator's — anything else is a proxy, a WAF or a gateway
answering in the store's place, and their answer is markup), the trimmed body says something at all,
and `restatesItsOwnStatus` does not fire.

**`restatesItsOwnStatus` is a rule about the whole class rather than a list of today's statuses**,
because it holds the trimmed body against `node:http`'s `STATUS_CODES` — the registry of reason
phrases a status line already carries — case-insensitively, since the worker writes sentence case
where the registry writes title case. It catches two of the three `c.text` refusals this route
writes: `Too many requests` is 429's `Too Many Requests` re-cased and `Payload too large` is 413's,
so neither is named in the CLI and a fourth refusal written the same way is suppressed the day it
ships. `Could not store this config` is not `Service Unavailable`, which is why the one refusal on
this route naming a cause is the one that survives. What earns a quote is not a body — it is a body
that adds something to the status printed beside it.

**So it degrades to the status alone** for a body that is absent, is not JSON and did not arrive as
text, is JSON that is not the envelope and did not arrive as text, arrived as text but is empty
after trimming, or arrived as text and only restates its own reason phrase. `refusalBody` wraps the
read as well as the parse — draining the stream can throw in its own right — and nothing below that
line is worth failing over, since the message being assembled is already a failure. The quote is
clipped to `EXPLANATION_BUDGET` (300) by `truncateText`, which strips terminal control sequences
before it measures: the
store's real refusal is one sentence of around 150 characters, so the budget holds that whole and a
second beside it, and past that a proxy or a captive portal has stopped explaining and started
painting the terminal. Same reasoning as `EXCERPT` in `read-piped-payload.ts`, at the length this
quote needs — that one bounds unreadable bytes shown so a user recognises them, this one bounds
prose the user has to be able to finish reading.

**Only the 400 carries an envelope**: the worker writes 409, 413, 429 and 503 with `c.text`, so the
zod-validator fall-through is its one JSON-bodied refusal and the other three are the prose arm's
whole population. `refusalMessage` tries both shapes against every status but 409 anyway — which
statuses answer in which shape is the deployed worker's business, and a body that is neither
degrades to the status for free.

**409 takes its own branch, ahead of any body read**, under `CONTRACT_MISMATCH`. It answers the
status sentence plus `OUT_OF_DATE_AGAINST_STORE`, and the branch is on the STATUS rather than on the
body because the status is the whole of the fact: what a 409 says is that the two ends name different
contract versions, and no body the store could send makes that more actionable from a terminal.

**The store's 409 body names a remedy a terminal cannot carry out**, which is why it is the one
refusal spoken in the CLI's own words. `refuseAnotherSeedVersion` in `apps/server/src/index.ts`
answers `Reload the page: …`, written for the caller that refusal was designed around — a browser tab
minting from a bundle older than the last deploy, for which one reload really is the whole fix. There
is no page here, and quoting it would be worse than the bare status, which at least does not send the
reader somewhere that does not exist. The fact underneath is the same one and from a terminal it is
actionable: `SEED_VERSION` is a `z.literal` imported from `@workspace/matrix/seed` and bundled into
the binary, so the version written here travels INSIDE the CLI — nothing about the configuration, the
directory it was read from or the pipe it arrived on can change it, and a newer CLI is the only thing
that can. It cannot be the payload's fault by either door: `read-piped-payload.ts` validates a piped
configuration against `installableSeedPayloadSchema` before this runs, and `config-to-seed.ts` writes
`SEED_VERSION` into one it builds, so a `v` this CLI does not itself name never reaches the wire.

**`OUT_OF_DATE_AGAINST_STORE` names no sub-command, deliberately twice over.** `publishSeedConfig` is
called by both `share` and `edit --ui`, so there is no one command to re-run; and an invocation
written after `@latest` is invisible to `src/cli/lib/__tests__/helpers/handed-out-invocations.ts`,
whose `HANDS_OUT` reader matches `${CLI_INVOKE_COMMAND}` followed by a SPACE — so naming one here
would hand out a command the gate over every handed-out invocation could not see. What it hands out
instead is `'${CLI_INVOKE_COMMAND}@latest'`, to re-run whichever command the user ran.
...
-> **165 passed**, 14 files. **This document owns these numbers.**
...
| `src/cli/lib/seed/seed-to-wizard.test.ts`                         | 13    | The four ways an agent reaches (or fails to reach) the result — named by the map, named only by an assignment, switched off, not real — plus the unwritable-pair refusal and the rows it must not trip on, the description carried or left off, and the decoded selection revalidated against this catalogue, where an unmet requirement is reported beside the skip it belongs to                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `src/cli/lib/seed/config-to-seed.test.ts`                         | 21    | Four groups: a round trip read back by the decoder, the envelope (including the carried content it was handed and the absent `external` key when it was handed none), what does not travel, and each refusal the contract cannot carry                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| `src/cli/lib/seed/fetch-seed.test.ts`                             | 16    | The inbound boundary: what it sends — the store's own path for an id, the CLI user-agent, no session credentials, and an id escaped so it cannot reshape the path — and what it answers with: the payload, an unknown id named rather than quoted at, an unreachable store, a body that is not JSON, a stored payload this contract no longer parses, and the refusal quoting — the store's own words beside the status for an integrity failure and for a refused read, the budget they are clipped to, the charset the worker really sends still reading as text, a body carrying terminal escapes rendered as text rather than obeyed, and the two bodies that degrade to the status alone (one that did not arrive as text, one that says nothing)                                                                                                                                                                                                                                                                                                                                                                |
| `src/cli/lib/seed/publish-seed.test.ts`                           | 19    | The outbound boundary: a payload the store's own POST schema would accept, the minted id, an id-less response reported rather than printed, an unreachable store named rather than thrown, and the 409, told the CLI is out of date rather than handed the worker's browser advice, with a bodiless 409 answered identically because the status is the whole of the fact. The refusal quoting is both arms and both directions: the envelope quoted beside the status and the budget it is clipped to; the plain-text arm quoted when its body names a cause its status line does not, and quoted still when the type arrives with the charset the worker really sends; a body carrying terminal escapes rendered as text rather than obeyed; and the bodies that degrade to the status alone — both statuses whose prose only restates their own reason phrase, so the rule reads as general rather than as one case, one restating around whitespace since the body is trimmed first, one announcing prose and carrying none, one answered in the store's place, one absent, and one whose JSON is not the envelope |
| `src/cli/lib/seed/read-piped-payload.test.ts`                     | 14    | `share --stdin`'s local gate, refused before the POST: a payload the contract takes, the scope-reach pair refused against the WRITE schema and taken once the sub-agent is pinned to project, the first unwritable pairs named with the rest counted, and the refusals kept distinct from one another — an empty pipe, whitespace as emptiness rather than malformed JSON, a body quoted back and clipped, JSON the contract will not take, another wire version, and the sanitising in both directions — a refused key named without the terminal escapes its producer put in it, an honest key still named in full, and a non-JSON body quoted back without the escapes inside it                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `src/cli/lib/seed/external-skills.test.ts`                        | 17    | Both content directions: seating (category placement, an entry the decode running immediately after can still read, per-entry scope, unselected content ignored, an undeclared category left unseated, sub-agent reach, the plugin refusal, the catalogue-id refusal naming every colliding id, and the `local` incumbent that is deliberately not one — a re-apply over this installation's own carried skill) and writing (nesting, global scope, the id rename, on-disk registration, a `doctor`-valid metadata.yaml, the repository's own fields kept, the `forkedFrom` stamp, and a path escaping the skill directory)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| `src/cli/lib/seed/seed-apply.test.ts`                             | 17    | Four groups: what a shared configuration is allowed to remove (a project-scoped skill, an inherited global skill and an inherited global sub-agent all left in the removal set; a skill written here kept at either scope; nothing added back for an entry the payload itself carries; a tombstone in neither half), an id this catalogue cannot place (kept at either scope, called authored where it is that too, and a tombstone still excluded), what a kept entry takes with it (stack rows, domains, and the row dropped with a removed sub-agent), and the shape handed on                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `src/cli/lib/seed/marketplace-ref-crossing.test.ts`               | 4     | The crossing the shared fixture cannot state about itself: the marketplace ref the editor mints, public and private, put to the CLI's own router `isLocalSource` and required to route remotely — with the bare form the dialog accepts held as the control that still reads as a local directory, and the payload pinned to the canonical constant rather than to anything merely remote-looking                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
**A body is filed only where the write contract would take it.** `isInstallable` asks
`installableSeedPayloadSchema` before `file` mints anything; a body that fails is left unfiled, so
the request falls through to `configHandlers` in `@workspace/api-mocks` and gets the worker's own 400. The store therefore restates no rule of its own — it imports the same schema the worker gates
with. A double that mints for any body is more permissive than the thing it stands in for, which
makes a suite green over a payload the real edge refuses.
`e2e/commands/seed-config-store-validation.e2e.test.ts` pins that with a raw `fetch` rather than a
spawned CLI, because `share` and `share --stdin` both refuse an unwritable payload locally before any
request leaves the process — so nothing routed through the CLI can reach the store's own refusal.
...
| `init-from-external-skills.e2e.test.ts`    | 4     | Skills the payload carries rather than names: one installed from the bytes the payload carries with no catalogue behind it, one routed to the scope its own entry names, the refusal to install a carried skill as a plugin with nothing written, and the refusal of a carried skill claiming an id the loaded catalogue already owns — proved against a whole-`$HOME` tree snapshot rather than a list of surfaces                                                                                                   |
...
| `e2e/interactive/edit-from.e2e.test.ts`    | 6     | The confirm, under a real PTY: the plan shown before anything is taken, the approved removal landing in config **and** on disk, a decline leaving the installation byte-identical, a removal reaching past this project shown under its own heading and then carried out, and the two disclosures — a skill written here the round trip never carried, and a skill it names that this catalogue cannot place                                                                                                          |

**60 executable specs across the 9 `init-from-*` files, `share.e2e.test.ts`, `edit-ui.e2e.test.ts`
