...
-> **163 passed**, 14 files. **This document owns these numbers.**
...
| `src/cli/lib/seed/external-skills.test.ts`                        | 15    | Both content directions: seating (category placement, an entry the decode running immediately after can still read, per-entry scope, unselected content ignored, an undeclared category left unseated, sub-agent reach, the plugin refusal) and writing (nesting, global scope, the id rename, on-disk registration, a `doctor`-valid metadata.yaml, the repository's own fields kept, the `forkedFrom` stamp, and a path escaping the skill directory)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
**59 executable specs across the 9 `init-from-*` files, `share.e2e.test.ts`, `edit-ui.e2e.test.ts`
