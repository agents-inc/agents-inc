...
`dependencies` — and that node's hash feeds the CLI's. Measured on turbo 2.10.8 (the row was filed
...
| Layer                     | Coupling                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| ------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Unit (`commands` project) | `cli-runner.ts` -> `run(args, { root: CLI_ROOT })` resolves commands from `dist/commands` (§8). Two things supply or police `dist/` here, and neither is redundant (a third, the `pretest` npm hook, was removed on 2026-08-23 — turbo already ordered the build and the hook was a second one racing it): `packages/cli/turbo.json`'s `test` -> `dependsOn: ["build"]` (without it, 205 tests across 17 files failed with exit 127 on a clean checkout), and `vitest.config.ts`'s `globalSetup` -> `vitest.global-setup.ts` -> `assertDistIsFresh` in `src/cli/lib/testing/dist-staleness.ts` (the hook is a three-line caller; the logic sits under `src/` so `tsc` and `eslint` reach it), which refuses any run — `npx vitest run <file>` included — whose `dist/` predates a tree compiled into it (`packages/cli/src` **or** `packages/matrix/src`, which tsup inlines — see §2). The guard exists because the first two are both bypassable and the failure they miss is a **false green**: see clean-code-standards 6.19 |
| E2E                       | `turbo` orders `build` ahead of `test:e2e`; specs spawn `node <repo>/bin/run.js`, whose `execute({ dir })` resolves the same `dist/commands`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
