/**
 * Entry point for the Vitest half of `bun run deps:check`.
 *
...
} from "./check-shared-vitest-config.ts";
...
function reportDiverged(verdicts: WorkspaceVerdict[]): void {
  console.error("Workspaces have diverged from the shared Vitest config:\n");
  ...
  console.error(
    [
      "",
      `Extend the config in packages/vitest-config and declare ${SHARED_CONFIG_PACKAGE},`,
