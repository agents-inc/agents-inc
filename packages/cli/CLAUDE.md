...
- NEVER use git worktrees (`isolation: "worktree"`). Reaffirmed by the owner 2026-09-03 — "I don't
  use worktrees" — after a rewrite briefly narrowed this to permit them on a compiled reviewing
  agent. That narrowing is withdrawn and no shipped agent carries the key. The CLI still SUPPORTS
  `isolation` in agent metadata, because Claude Code's frontmatter documents it and a consuming
  project may want it; supporting a key and using it are separate decisions, and this rule governs
  the second.
...
- NEVER leave the key order of a nested record to whoever assembled it when that record is serialised. Canonicalise it **once in the writer**, not once per producer: a producer that REPLACES another's output rather than merging with it also replaces its ordering, and nothing downstream can tell. `canonicalizeStackOrder` in `packages/compile/src/config-source.ts` is the shape — run from `cleanForEmission`, sub-agent keys in code-unit name order and each sub-agent's category keys through `byCategoryDeclarationOrder` in `packages/compile/src/catalog.ts`, which is where the rule itself lives so the builder and the writer cannot disagree about it. Both moved out of this package with the renderer extraction and neither is re-exported here — the builder that calls the comparator is `inCanonicalCategoryOrder` in `packages/compile/src/seed-to-config.ts`, so no call site is left in the CLI. The CLI's own copy of the comparator, in `lib/matrix/matrix-provider.ts`, outlived its last caller and was deleted rather than kept: a centralised rule declared a second time is a plausible-looking definition for a future caller to reach, and nothing in the gates can see a symbol that nothing invokes. The adjacent rule was already right and one level too shallow: `cleanForEmission` canonicalised the config's TOP-LEVEL field order for word-for-word this reason, while `stack` — a record of records — stayed as producer-dependent as the fields above it. Several modules assemble a stack — enumerate them with `grep -rn "Partial<Record<AgentName, StackAgentConfig>>" --include='*.ts' --include='*.tsx' src e2e` — and only the wizard's ordered anything, so a shared configuration rebuilt by `init --from` matched field for field and compiled a `web-developer.md` with two rows of its skill-activation table swapped — `buildAgentTemplateContext` splits `agent.skills` into preloaded and dynamic PRESERVING order, so the key order in `config.ts` IS the order the sub-agent is handed. Use `bytewise` from `utils/string.ts` or a bare `.sort()`, never `localeCompare` — see `clean-code-standards.md` 17.3.
...
- ALWAYS delegate implementation and test code to sub-agents. Tell them to read CLAUDE.md. State the git rule's both halves: read-only git (`status`, `log`, `show`, `diff`, `blame`, `stash list`) is theirs to use, and every writing command is the user's.
