...
describe("generateMetadataYaml", () => {
  ...
  it("should include yaml-language-server schema comment at the top", () => {
    const content = generateMetadataYaml("my-skill", "@local", "local", TEST_CONTENT_HASH, "web");
    // The URL comes from SCHEMA_PATHS rather than a literal: it moved once already when the CLI
    // became packages/cli, and a hard-coded copy here failed while the source was correct.
    expect(content.split("\n")[0]).toBe(
      `# yaml-language-server: $schema=${SCHEMA_PATHS.customMetadata}`,
    );
  });
