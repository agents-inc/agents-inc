...
function groupStacks(stacks: GroupableStack[]): StackGroup[] {
  ...
  const labelled = Object.entries(byLabel).sort(([a], [b]) => compareGroupLabels(a, b));
  if (labelled.length === 0) {
    return [{ label: "", items: ungrouped }];
  }

  const groups = labelled.map(([label, stacksInGroup]) => ({
    label,
    items: stacksInGroup.map(toStackItem),
  }));
  ...
  return (
    <Box flexShrink={0}>
      <Text>
        ...
        <Text {...(isFocused && { color: CLI_COLORS.PRIMARY })} bold={isFocused}>
          {" "}
          {item.name}
        </Text>
        <Text dimColor>
