...
4. **One accent colour.** Amber marks what the user deliberately chose or changed. Hover states stay
   ...
   second signal colour: a skill the selection has ruled out recedes rather than reddening — see §9.
...
| -------- | ---------------------------------------- | ------- | ------------------------------------------------------------------------------------------------------------------------------------- |
| `domain` | `z.enum(DOMAINS).nullable().catch(null)` | `null`  | An ANCHOR rather than a filter (EDITOR-79): it scrolls the column to that section on arrival, and every domain renders at every value |
...
| `button.tsx`         | `outline` · `primary` · `block` · `full` · `action`                             |
...
| `glyph.tsx`          | The one icon set — 1.75 stroke, butt caps, miter joins                          |
| `menu.tsx`           | The roster's flat grouping menu                                                 |
...
| `.../stack-grid.tsx`                | 4-across stack lattice: the reset cell — `Start from scratch`, or `Clear and start from scratch` with its removal count once anything is selected — the saved snapshot when there is one, then the catalogue's stacks |
| `.../filter-bar.tsx`                | Sticky/stuck bar + search + `+ Add skill`, and the domain strip beneath it. Nothing else shares the strip's row: `selected` is a section-level control on the skills hinge, and clearing is the first stack cell      |
...
| #   | Question               | Decision                                                                                                                                                                                                                                                           |
| --- | ---------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 1   | **Domain set**         | **Retired 2026-09-05.** Both halves of it are gone: the strip is equal cells over EVERY catalogue domain, so there is no shorter list to chip and no chip to put it in.                                                                                            |
| 2   | **Sub-agent taxonomy** | Real agents from the catalog — 18 across five groups, ragged — not the design's clean 4 × 4 of 16.                                                                                                                                                                 |
| 3   | **Domain hierarchy**   | **Superseded twice.** The in-column title is gone (Phase F), and the tab that replaced it holds ONE type size at every state (2026-09-05) — a 25px active label reflowed the column, which moved the sections the scroll spy reads to decide which tab is current. |
...
| `incompatible` | Recedes to the column colour at full opacity (`bg-column`), plus `aria-disabled` and the reason on `title`. Never hidden. |
...
| **Incompatible cell**                | `.skc.dis{background:#faf8f2}` — it stops being white and sinks into the column, and nothing else                                             | The same recede, plus `aria-disabled` and a reason on `title`                                                           | The treatment matches the design exactly. It was `opacity:.4` on both sides until 2026-09-05; the fade put the skill's name at roughly 2.1:1 and took the cell's own hairlines with it, so the lattice broke and it read as a rendering fault rather than a state. What the design still has no answer for is _why_ a cell is out, and that a mouse-only signal leaves the state unreadable to anything else.                                                                                                                                                                                                                                                                                                                                                   |
| **Roster footer**                    | Save / Share / `<> Preview` as three equal outlined cells in one row, then Install full width carrying the counts                             | The same four, in that order                                                                                            | Save snapshots the selection into the stack grid; Share is a shipped Cloudflare KV round trip. Both are disabled with nothing selected, and Save's label never moves — the grid cell appearing is the feedback, and a button that renames itself cannot be clicked twice in a row.                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| **Where an added skill lands**       | "lands in Uncategorized" — never mocked                                                                                                       | A category the user picks at add time, from the loaded catalogue's own list                                             | A placement is a decision, and one guessed from a repository name is a multi-tier fallback dressed as a feature. With a real category the skill renders under a real domain and is reached by the strip like its neighbours, gets a sub-agent reach from the shared resolver and is judged by the same semantics as they are — none of which an orphan section can offer.                                                                                                                                                                                                                                                                                                                                                                                       |
