...
export function buildCompileAgents(
  ...
  const globalSkillIds = new Set(
    activeSkills.filter((s) => s.scope === "global").map((s) => s.id)
  )
...
const globalSkills = config.skills.filter(
  (s) => s.scope === "global" && !s.excluded
)
...
// Agents: same pattern
const globalAgents = config.agents.filter(
  (a) => a.scope === "global" && !a.excluded
)
...
if (deselectedConfig?.scope === "global") {
  ...
  updatedConfigs = state.skillConfigs.map((sc) =>
    sc.id === id ? { ...sc, excluded: true } : sc
  )
} else {
...
skillConfigs
  ...
  .map((sc) =>
    removed.has(sc.id) && sc.scope === "global" ? { ...sc, excluded: true } : sc
  )
```
...
const merged = sorted.map(
  (name) => existing.get(name) ?? { name, scope: "global" }
)
